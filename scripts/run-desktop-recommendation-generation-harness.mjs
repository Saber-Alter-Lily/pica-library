...
async function verifyDesktop(baseUrl) {
    ...
    if (base.protocol !== 'http:')
        throw new Error('J7 generation requires a loopback HTTP Desktop URL')
    const host = base.hostname.replace(/^\[|\]$/g, '')
    if (!['127.0.0.1', 'localhost', '::1'].includes(host))
        throw new Error('J7 generation refuses non-loopback Desktop URLs')
    ...
    if (status?.application !== 'Pica Library' || !status?.configured)
        throw new Error(
            'J7 generation requires an already-configured Pica Library Desktop'
        )
}
...
async function main() {
    ...
    const toolRoot = fs.mkdtempSync(
        path.join(os.tmpdir(), 'pica-j7-real-provider-playwright-')
    )
    try {
        fs.writeFileSync(
    ...
    process.stdout.write(
        'J7 real generation benchmark completed. No credentials were read or exported by the runner.\n'
    )
}
