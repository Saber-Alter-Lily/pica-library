...
| RT-11 | Recommendation V5 shadow retrieval | detached service-owned Desktop background task preserving the existing computation core | candidate pool/audit telemetry persists only after the final safe checkpoint; compact task summary is not a second evidence authority | explicit confirmation + status + pause/resume/cancel + provider-route/analysis checkpoints + UI reattach | NET-P, CPU, DB-R/W | **H2A COMPLETE (PR #116)** |
...
| RT-13 | Visual QC / author atlas / style-family evaluation | synchronous service calculations for QC/gates; indexing itself is RT-02 | read-only output built from stored embeddings/catalog | manual/advanced only; bounded timing registry now records real invocation latency/input size | CPU, DB-R | **H2B MEASUREMENT ACTIVE** — instrumentation and a deterministic synthetic scaling harness exist; foreground threshold remains intentionally unset until representative real-use timings are collected |
| RT-14 | Work Identity audit/evidence refresh | ordinary evidence refresh is a detached service task over the checkpointable async audit; synchronous builder remains the deterministic baseline | evidence table is durable; incomplete scans stay in memory and persist only after the final checkpoint; automatic binding disabled | authoritative loading/bucketing/comparing/persisting status plus pause/resume/cancel and Web reattach | CPU, DB-R/W | **H2C CANDIDATE** — full-domain correctness is preserved without a request-owned pair scan; the initial SQLite full-catalog materialization and review/plan calculations still need later P2-D/P2-C measurement |
...
### F-02 — Maintenance update scan request ownership / 5000-item truncation
**Priority: RESOLVED for H1 by PR #113**

Completed:
- ordinary full scans no longer stay attached to one long HTTP request;
- the full scan exposes authoritative status plus pause/resume/cancel;
- checkpoints run before and after each bounded Provider request;
- discovery uses the complete set of downloaded comic IDs queried directly from SQLite;
- the former `listComics({ limit: 5000 })` correctness truncation is removed;
- full scans remain review-first and do not auto-queue update jobs.

Remaining outside H1:
- task state is process-local rather than restart-durable;
