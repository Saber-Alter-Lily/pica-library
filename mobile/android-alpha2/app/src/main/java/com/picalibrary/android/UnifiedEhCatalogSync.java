...
/** Merges public E-H metadata into the shared mobile catalog. */
final class UnifiedEhCatalogSync {
    private UnifiedEhCatalogSync(){}

    static UnifiedCatalogStore.Entry merge(Context context,EhClient.Comic comic){UnifiedCatalogStore.Snapshot snapshot=UnifiedCatalogStore.load(context);UnifiedCatalogStore.Entry entry=apply(snapshot,comic);UnifiedCatalogStore.save(context,snapshot);return entry;}
    static void mergeAll(Context context,List<EhClient.Comic> comics){UnifiedCatalogStore.Snapshot snapshot=UnifiedCatalogStore.load(context);for(EhClient.Comic comic:comics)apply(snapshot,comic);UnifiedCatalogStore.save(context,snapshot);}
