...
public final class PicaBootstrapWorker extends Worker {
    ...
    @NonNull @Override public Result doWork(){
        try{
            ...
            PicaClient client=new PicaClient(app);List<PicaClient.Comic> favorites=client.favoritesAll((page,pages,fetched,total)->{if(isStopped())throw new InterruptedException("Pica 收藏同步已停止");setProgressAsync(new Data.Builder().putString(KEY_PHASE,"正在读取 Pica 收藏 · 第 "+page+" / "+pages+" 页").putInt(KEY_DONE,fetched).putInt(KEY_TOTAL,total).build());});
