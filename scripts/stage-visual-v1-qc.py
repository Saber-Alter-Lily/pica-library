...
service_path = 'src/library/service.ts'
...
old_similar = """    similarVisualStyle(comicId: string, limit = 20) {
...
"""
new_similar = """    similarVisualStyle(comicId: string, limit = 20) {
        const embeddings = this.database.listVisualEmbeddings()
        const preferred = new Map<string, VisualEmbeddingRecord>()
        for (const item of embeddings) {
            if (
...
"""
if old_similar not in service:
    raise SystemExit('similarVisualStyle patch anchor not found')
service = service.replace(old_similar, new_similar, 1)
write(service_path, service)

runtime_path = 'web/visual-runtime.js'
...
if "./visual-qc-beta.js" not in runtime:
    runtime = runtime.rstrip() + loader
write(runtime_path, runtime)

visual_qc = r'''const A88_RATINGS_KEY = 'pica-visual-qc-ratings-v1'
...
'''
write('web/visual-qc-beta.js', visual_qc)
...
docs_path = 'docs/RECOMMENDATION_V4_VISUAL_BETA.md'
docs = read(docs_path)
section = r'''
...
'''
if '## Visual V1 frozen-index QC' not in docs:
    docs = docs.rstrip() + section + '\n'
write(docs_path, docs)

print('Visual V1 QC patch staged successfully')
