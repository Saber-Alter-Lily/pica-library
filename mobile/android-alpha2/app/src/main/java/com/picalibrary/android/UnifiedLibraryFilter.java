...
final class UnifiedLibraryFilter {
    ...
    static final class Spec {
        ...
        Spec copy(){Spec next=new Spec();next.text=text;next.sources.addAll(sources);next.authors.addAll(authors);next.tags.addAll(tags);next.categories.addAll(categories);next.favoriteOnly=favoriteOnly;next.shelfOnly=shelfOnly;next.finished=finished;next.tagMode=tagMode;next.sort=sort;return next;}
        int activeCount(){int count=sources.isEmpty()?0:1;if(favoriteOnly)count++;if(shelfOnly)count++;if(finished!=null)count++;if(!authors.isEmpty())count++;if(!tags.isEmpty())count++;if(!categories.isEmpty())count++;if(sort!=Sort.LATEST)count++;return count;}
