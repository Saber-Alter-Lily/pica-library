...
public final class DisclaimerActivity extends LocaleAwareActivity {
    ...
    static void requireBeforePairing(Activity activity,Intent original){if(accepted(activity))return;Intent gate=new Intent(activity,DisclaimerActivity.class);gate.putExtra("next","pair");if(original!=null&&original.getData()!=null)gate.putExtra("nextData",original.getData().toString());activity.startActivity(gate);activity.finish();}

    @Override public void onCreate(Bundle saved){super.onCreate(saved);Ui.applyWindow(this);boolean review=getIntent().getBooleanExtra("reviewOnly",false);if(accepted(this)&&!review){continueIntoApp();return;}render(review);}
    private void render(boolean review){
        ScrollView scroll=new ScrollView(this);scroll.setBackgroundColor(Ui.BG);scroll.setFillViewport(true);scroll.setClipToPadding(false);scroll.setOnApplyWindowInsetsListener((v,insets)->{v.setPadding(0,insets.getSystemWindowInsetTop(),0,insets.getSystemWindowInsetBottom());return insets;});
    ...
    private void continueIntoApp(){String next=getIntent().getStringExtra("next");if("pair".equals(next)){Intent pair=new Intent(this,PairingActivity.class);String data=getIntent().getStringExtra("nextData");if(data!=null&&!data.isEmpty())pair.setData(android.net.Uri.parse(data));startActivity(pair);}else startActivity(new Intent(this,HomeActivity.class));finish();}
}
