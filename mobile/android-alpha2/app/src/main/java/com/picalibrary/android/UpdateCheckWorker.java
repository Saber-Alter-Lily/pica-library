...
public final class UpdateCheckWorker extends Worker {
    ...
    private static void notifyUpdate(Context context,UpdateClient.Info info){
        android.content.SharedPreferences prefs=context.getSharedPreferences(PREF,Context.MODE_PRIVATE);long now=System.currentTimeMillis();if(prefs.getInt("lastPromptedVersionCode",-1)==info.versionCode&&now-prefs.getLong("lastPromptedAt",0)<12L*60*60*1000)return;
