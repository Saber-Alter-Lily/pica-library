...
final class FavoriteCacheStore {
    ...
    static Snapshot syncFromDesktop(Context context,boolean covers,Progress progress,Control control) throws Exception {
        List<BridgeClient.ComicItem> items=fetchAll(context,control);if(control!=null&&control.stopped())throw new InterruptedException("收藏导入已停止");save(context,items,false);try{ShelfStore.syncFromDesktop(context);}catch(Exception ignored){}if(progress!=null)progress.update(0,items.size(),"收藏与书架元数据已导入");boolean allCovers=false;
