...
- **G15 real process-death gate:** a debug-only WorkManager probe is kept RUNNING while the host executes `adb shell am force-stop com.picalibrary.android.dev`. Because a force-stopped package must not spontaneously execute background work, the host first confirms process death, then re-enters through the normal launcher and requires probe runCount >= 2 in a fresh process before the second instrumentation verifies active/PAUSED recovery state, one-card Task Center reconstruction and cancelled-history non-resurrection.
...
- G15 closes that named evidence gap with host-side `adb shell am force-stop` while a debug-only WorkManager probe is actually RUNNING, followed by a normal launcher re-entry and a fresh verification instrumentation.
- Force-stop is treated as a real stopped-state boundary: the app is not expected to resurrect background work while stopped. After explicit launcher re-entry, the host requires a new target PID and probe runCount >= 2 before verification, proving WorkManager reconstructed unfinished work rather than merely reading the old database row.
