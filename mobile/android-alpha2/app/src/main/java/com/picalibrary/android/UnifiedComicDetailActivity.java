...
public final class UnifiedComicDetailActivity extends Activity {
    ...
    private String availabilityLabel(){List<String> values=new ArrayList<>();if(entry.phoneDownloaded)values.add("手机");if(entry.desktopDownloaded)values.add("电脑");if(entry.remoteAvailable)values.add("WebDAV");if(entry.picaAvailable||PicaAccountStore.load(this).configured())values.add("Pica 在线");if(values.isEmpty())values.add("当前只有元数据");return String.join(" · ",values);}
    ...
    private void resolveSource(){
        worker.submit(()->{
            ...
            if(PicaAccountStore.load(this).configured())try{
                PicaClient client=new PicaClient(this);PicaClient.Comic online=client.comic(entry.id);if(!online.id.isEmpty()){entry=UnifiedPicaCatalogSync.merge(this,online);ReaderSource source=new PicaReaderSource(this);List<BridgeClient.ChapterItem> chapters=source.chapters(entry.id);if(!chapters.isEmpty()){showResolved("pica","Pica 在线",chapters);return;}}
    ...
    private void showResolved(String sourceKind,String label,List<BridgeClient.ChapterItem> chapters){runOnUiThread(()->{if(destroyed)return;loading.setVisibility(View.GONE);sourceState.setText("当前阅读来源："+label+" · "+chapters.size()+" 个章节");TextView section=Ui.text(this,"可读章节",20,Ui.TEXT,true);section.setPadding(0,Ui.dp(this,14),0,Ui.dp(this,6));content.addView(section);for(BridgeClient.ChapterItem chapter:chapters){LinearLayout row=Ui.card(this);row.setPadding(Ui.dp(this,15),Ui.dp(this,12),Ui.dp(this,15),Ui.dp(this,12));row.addView(Ui.text(this,chapter.title,15.5f,Ui.TEXT,true));row.addView(Ui.text(this,"pica".equals(sourceKind)?"在线读取 · "+label:chapter.downloadedPictures+" 页 · "+label,11.5f,Ui.MUTED,false));row.setOnClickListener(v->openReader(sourceKind,chapter.id));content.addView(row);}});}
