...
public final class AccountSourcesActivity extends Activity {
    ...
    private void renderContent(){if(content==null)return;probeSerial++;content.removeAllViews();boolean pica=PicaAccountStore.load(this).configured(),eh=EhAccountStore.load(this).configured();content.addView(SettingsRow.row(this,"Pica",pica?"已连接":"未连接",v->startActivity(new Intent(this,PicaAccountActivity.class))));LinearLayout card=Ui.card(this);card.setOnClickListener(v->startActivity(new Intent(this,EhAccountActivity.class)));LinearLayout head=new LinearLayout(this);head.setGravity(Gravity.CENTER_VERTICAL);head.addView(Ui.text(this,"E-Hentai / ExHentai",16,Ui.TEXT,true),new LinearLayout.LayoutParams(0,-2,1));TextView arrow=Ui.text(this,"›",24,Ui.MUTED,false);head.addView(arrow);card.addView(head);card.addView(SettingsRow.statusLine(this,"E-H 账号",eh?"已连接":"未连接"));exhState=Ui.text(this,eh?"检查中":"未检测",13,Ui.MUTED,true);card.addView(SettingsRow.statusLine(this,"ExH",exhState));content.addView(card);if(eh)probeExh();}
