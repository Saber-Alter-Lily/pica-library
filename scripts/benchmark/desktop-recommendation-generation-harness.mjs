...
function parseOptions() {
    ...
    if (optionValue('confirm-regeneration') !== 'YES')
        throw new Error(
            'J7 real generation changes the current recommendation cycle; pass --confirm-regeneration=YES explicitly'
        )
    const base = new URL(rawBaseUrl)
    if (base.protocol !== 'http:')
        throw new Error('J7 generation accepts only loopback HTTP Desktop URLs')
    if (base.username || base.password)
        throw new Error('Credentials in J7 generation base URL are forbidden')
    const host = base.hostname.replace(/^\[|\]$/g, '')
    if (!['127.0.0.1', 'localhost', '::1'].includes(host))
        throw new Error('J7 generation accepts only loopback Desktop URLs')
    base.pathname = '/'
    base.search = ''
    base.hash = ''
    return {
...
function playwright() {
    ...
    if (!toolRoot)
        throw new Error(
            'PICA_PLAYWRIGHT_TOOL_ROOT is required; use the J7 generation runner so Playwright stays outside project dependencies'
        )
...
async function initialRecommendationReady(page, timeoutMs) {
    ...
    if (!existing?.activeCycleId || existing?.buildingCycleId)
        throw new Error(
            'J7 real generation requires one existing usable managed V3 cycle before any recommendation UI action'
        )
    ...
    if (
        ...
        throw new Error(
            'J7 recommendation precondition changed before the measured regeneration'
        )
    return existing
}
    ...
    try {
        ...
        while (Date.now() < deadline) {
            ...
            if (
                ...
                throw new Error(
                    'J7 regeneration completed without observing a real provider-network resource lease'
                )
            }
            if (
                status?.activeCycleId &&
                ...
                if (pageErrors.length)
                    throw new Error(
                        `Web page error during J7 real regeneration: ${pageErrors.join(' | ')}`
                    )
                await context.close()
                return {
            ...
            if (/失败|failed|超时|timeout/i.test(message))
                throw new Error(`J7 real regeneration failed: ${message}`)
            await new Promise((resolve) => setTimeout(resolve, 100))
        }
        throw new Error('J7 real regeneration timed out')
    } finally {
        await browser.close()
    }
}
...
async function main() {
    ...
    const report = {
        schemaVersion: 1,
        benchmark: 'p2-j7-desktop-recommendation-real-generation',
