...
final class UnifiedLibraryFilterDialog {
    ...
    static void show(Activity activity,UnifiedLibraryFilter.Spec current,UnifiedLibraryFilter.Facets facets,Apply apply){
        ...
        authors.setOnClickListener(v->showFacetPicker(activity,"选择作者",facets.authors,working.authors,()->authors.setText(facetLabel(LocalizedText.ui("作者"),working.authors.size()))));
        tags.setOnClickListener(v->showFacetPicker(activity,"选择标签",facets.tags,working.tags,()->tags.setText(facetLabel(LocalizedText.ui("标签"),working.tags.size()))));
        categories.setOnClickListener(v->showFacetPicker(activity,"选择分类",facets.categories,working.categories,()->categories.setText(facetLabel(LocalizedText.ui("分类"),working.categories.size()))));
        Button more=facetButton(activity,"更多筛选",advancedCount(working));root.addView(more);more.setOnClickListener(v->showAdvanced(activity,working,()->more.setText(facetLabel(LocalizedText.ui("更多筛选"),advancedCount(working)))));

        AlertDialog dialog=new AlertDialog.Builder(activity).setTitle(LocalizedText.ui("筛选")).setView(root).setNegativeButton(LocalizedText.ui("取消"),null).setNeutralButton(LocalizedText.ui("清除"),null).setPositiveButton(LocalizedText.ui("应用"),null).create();
