...
export class GitHubAccountAuthService {
    ...
    async start() {
        ...
        if (!response.ok)
            throw new Error(
                String(value.error_description ?? value.error ?? `GitHub 登录启动失败（HTTP ${response.status}）`)
            )
        ...
        if (!deviceCode || !userCode || !/^https:\/\/github\.com\//.test(verificationUri))
    ...
    async poll(flowIdInput: unknown): Promise<
    ...
    > {
        ...
        if (!pending) throw new Error('GitHub 登录会话不存在或已过期，请重新开始')
        const now = Date.now()
        ...
        if (now < pending.nextPollAt)
            return { state: 'pending', pollAfterMs: pending.nextPollAt - now }
        ...
        if (!response.ok)
            throw new Error(
                String(value.error_description ?? value.error ?? `GitHub 登录失败（HTTP ${response.status}）`)
            )
    ...
    private async verifyAuthenticatedStar(token: string): Promise<GitHubVerifiedIdentity> {
        const headers = {
        ...
        if (!identityResponse.ok)
            throw new Error(`GitHub 账号身份读取失败（HTTP ${identityResponse.status}）`)
        const githubUser = String(identity.login ?? '').trim()
        const githubUserId = Number(identity.id ?? 0)
        if (!githubUser || !Number.isSafeInteger(githubUserId) || githubUserId <= 0)
        ...
        if (starResponse.status === 404)
            throw new Error(`已登录 GitHub 账号 ${githubUser}，但该账号尚未 Star Pica Library`)
        if (starResponse.status !== 204)
            throw new Error(`GitHub Star 身份验证失败（HTTP ${starResponse.status}）`)
    ...
    private prune() {
        ...
        while (this.pending.size > 20) this.pending.delete(this.pending.keys().next().value as string)
    }
}
