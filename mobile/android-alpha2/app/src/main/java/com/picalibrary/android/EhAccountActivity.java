...
public final class EhAccountActivity extends Activity {
    ...
    private void refreshExh(boolean force){if(!EhClient.accountAvailable(this)){applyExh(EhCapabilityStore.load(this));return;}final int id=++probeSerial;if(force&&exhState!=null){exhState.setText("检查中");exhState.setTextColor(Ui.MUTED);}EhCapabilityStore.refreshAsync(this,force,value->{if(id!=probeSerial||isDestroyed()||exhState==null)return;applyExh(value);});}
