...
public class ComicDetailActivity extends Activity {
    ...
    private void load(){
        ...
        ImageRepository.load(this,cover,coverPath,0xffebe8ef);
        LinearLayout copy=new LinearLayout(this);copy.setOrientation(LinearLayout.VERTICAL);copy.setPadding(Ui.dp(this,15),Ui.dp(this,3),0,0);TextView titleView=Ui.text(this,title==null?"未命名漫画":title,21,Ui.TEXT,true);titleView.setMaxLines(4);copy.addView(titleView);Ui.gap(copy,this,8);copy.addView(Ui.text(this,author==null?"未知作者":author,14,Ui.MUTED,false));Ui.gap(copy,this,12);copy.addView(Ui.pill(this,"电脑已下载 "+downloadedPictures+" 页",Ui.PRIMARY_SOFT,Ui.PRIMARY));hero.addView(copy,new LinearLayout.LayoutParams(0,-2,1));content.addView(hero);
        TextView section=Ui.text(this,"已下载章节",20,Ui.TEXT,true);content.addView(section);TextView hint=Ui.text(this,"只显示电脑端已经完整或部分下载、可直接读取的章节",12,Ui.MUTED,false);hint.setPadding(0,Ui.dp(this,3),0,Ui.dp(this,8));content.addView(hint);
        loading=new ProgressBar(this);content.addView(loading);
        new Thread(()->{try{List<BridgeClient.ChapterItem> all=BridgeClient.chapters(this,comicId);List<BridgeClient.ChapterItem> downloaded=new ArrayList<>();for(BridgeClient.ChapterItem c:all)if(c.downloadedPictures>0)downloaded.add(c);Collections.sort(downloaded,Comparator.comparingInt((BridgeClient.ChapterItem c)->c.order).reversed());runOnUiThread(()->showChapters(downloaded));}catch(Exception e){runOnUiThread(()->{loading.setVisibility(View.GONE);content.addView(Ui.text(this,"章节读取失败："+e.getMessage(),14,0xffa43b3b,false));});}}).start();
    }
    ...
    private void showChapters(List<BridgeClient.ChapterItem> chapters){
        ...
        for(BridgeClient.ChapterItem c:chapters){LinearLayout row=Ui.card(this);row.setPadding(Ui.dp(this,15),Ui.dp(this,12),Ui.dp(this,15),Ui.dp(this,12));LinearLayout top=new LinearLayout(this);top.setGravity(Gravity.CENTER_VERTICAL);TextView name=Ui.text(this,c.title,15.5f,Ui.TEXT,true);top.addView(name,new LinearLayout.LayoutParams(0,-2,1));top.addView(Ui.text(this,"›",22,Ui.MUTED,false));row.addView(top);row.addView(Ui.text(this,"已下载 "+c.downloadedPictures+" 页",11.5f,Ui.MUTED,false));row.setOnClickListener(v->openReader(c));content.addView(row);}
    }

    private void openReader(BridgeClient.ChapterItem c){Intent i=new Intent(this,ReaderActivity.class);i.putExtra("comicId",comicId);i.putExtra("title",title);i.putExtra("episodeId",c.id);startActivity(i);}
}
