...
public final class EhAccountActivity extends Activity {
    ...
    private void probe(){if(!EhAccountStore.load(this).configured()){if(exhState!=null){exhState.setText("需登录");exhState.setTextColor(Ui.MUTED);}return;}final int id=++probeSerial;if(exhState!=null){exhState.setText("检查中");exhState.setTextColor(Ui.MUTED);}new Thread(()->{String value=new EhClient(this).probeExH();runOnUiThread(()->{if(id!=probeSerial||isDestroyed()||exhState==null)return;String label="AVAILABLE".equals(value)?"可用":"NETWORK_ERROR".equals(value)?"网络异常":"无权限";exhState.setText(label);exhState.setTextColor(SettingsRow.statusColor(label));});}).start();}
