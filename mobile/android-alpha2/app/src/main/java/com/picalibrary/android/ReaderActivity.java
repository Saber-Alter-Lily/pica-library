...
public class ReaderActivity extends Activity {
    ...
    private void render() {
        ...
        root.setOnApplyWindowInsetsListener((v, insets) -> {
            top.setPadding(0, insets.getSystemWindowInsetTop(), 0, 0);
            bottom.setPadding(0, 0, 0, insets.getSystemWindowInsetBottom()); return insets;
        });
    ...
    private void loadInitial() {
        ...
        chapterRequest = metadata.submit(() -> {
            try {
                List<BridgeClient.ChapterItem> available = source.chapters(comic);
                ...
                boolean found = false; for (BridgeClient.ChapterItem c : available) if (c.id.equals(selected)) found = true;
                if (!found) selected = available.get(0).id;
                BridgeClient.ChapterData data = source.chapter(comic, selected);
                main.post(() -> { if (valid(serial)) { chapters = available; apply(data, -1); } });
    ...
    private void failChapter() {
        loading.setVisibility(View.GONE); retry.setVisibility(View.VISIBLE); setChrome(true);
        counter.setText(source instanceof RemoteReaderSource ? "无法读取章节，请检查 WebDAV 或本地缓存后重试" : "无法读取章节，请检查电脑连接后重试");
    }
    private void loadChapter(String id, int start) {
        save(true); desiredChapter = id; final int serial = ++generation;
        if (chapterRequest != null) chapterRequest.cancel(true);
        chapterReady = false; loading.setVisibility(View.VISIBLE); retry.setVisibility(View.GONE); clearPages();
        chapterRequest = metadata.submit(() -> {
            try {
                BridgeClient.ChapterData data = source.chapter(comic, id);
                main.post(() -> { if (valid(serial)) apply(data, start); });
            } catch (Exception e) { main.post(() -> { if (valid(serial)) failChapter(); }); }
        });
    }
    private void apply(BridgeClient.ChapterData data, int start) {
        ...
        heading.setText((title == null ? "漫画" : title) + " · " + data.episode.title);
        loading.setVisibility(View.GONE); chapterReady = true; seek.setMax(Math.max(0, pages.size() - 1));
        buildPages(); updateCounter();
        if (pages.isEmpty()) { counter.setText("该章节尚无可读取页面"); setChrome(true); }
    }
    private void clearPages() {
        if (pager != null) pager.setAdapter(null); if (continuous != null) continuous.setAdapter(null);
        for (Holder holder : new ArrayList<>(holders)) holder.cancel();
        holders.clear(); canvas.removeAllViews(); pager = null; continuous = null;
    }
    private void buildPages() {
        rebuilding = true; int restore = index; clearPages(); Pages adapter = new Pages();
        if (mode == 2) {
            continuous = new RecyclerView(this); LinearLayoutManager layout = new LinearLayoutManager(this);
            continuous.setLayoutManager(layout); continuous.setItemAnimator(null); continuous.setAdapter(adapter);
            continuous.addOnScrollListener(new RecyclerView.OnScrollListener() {
                public void onScrolled(RecyclerView v, int dx, int dy) {
                    int p = layout.findFirstVisibleItemPosition(); if (!rebuilding && p >= 0 && p != index) selected(p);
                }
            });
            canvas.addView(continuous, new FrameLayout.LayoutParams(-1, -1)); layout.scrollToPositionWithOffset(restore, 0);
        } else {
            pager = new ViewPager2(this); pager.setLayoutDirection(mode == 1 ? View.LAYOUT_DIRECTION_RTL : View.LAYOUT_DIRECTION_LTR);
            pager.setOffscreenPageLimit(1); pager.setAdapter(adapter);
            pager.registerOnPageChangeCallback(new ViewPager2.OnPageChangeCallback() {
                public void onPageSelected(int p) { if (!rebuilding) selected(p); }
            });
            canvas.addView(pager, new FrameLayout.LayoutParams(-1, -1)); pager.setCurrentItem(restore, false);
        }
        index = restore; rebuilding = false;
    }
    private void selected(int p) { index = ReaderPolicy.clampPage(p, pages.size()); updateCounter(); save(false); }
    private void updateCounter() { if (!dragging) { seek.setProgress(index); counter.setText((pages.isEmpty() ? 0 : index + 1) + " / " + pages.size() + (source instanceof RemoteReaderSource ? " · 云端/缓存" : " · 电脑/缓存")); } }
    private void save(boolean flush) {
        if (!chapterReady || pages.isEmpty()) return;
        boolean displayed = false; for (Holder h : holders) if (h.position == index && h.loaded) displayed = true;
        if (!displayed) return;
        progress.save(comic, chapter, index, flush); main.removeCallbacks(syncProgress);
        if (flush) progress.sync(); else main.postDelayed(syncProgress, 1200);
    }
    private void go(int p, boolean smooth) {
        if (!chapterReady || pages.isEmpty()) return; p = ReaderPolicy.clampPage(p, pages.size());
        if (pager != null) pager.setCurrentItem(p, smooth);
        else if (continuous != null) { ((LinearLayoutManager)continuous.getLayoutManager()).scrollToPositionWithOffset(p, 0); selected(p); }
    }
    private void flip(int direction) {
        if (!chapterReady || pages.isEmpty()) return;
        if (mode == 2 && continuous != null && continuous.canScrollVertically(direction)) { continuous.smoothScrollBy(0, direction * (int)(continuous.getHeight() * .85f)); return; }
        int next = index + direction;
        if (next >= 0 && next < pages.size()) { go(next, true); return; }
        int current = -1; for (int i = 0; i < chapters.size(); i++) if (chapters.get(i).id.equals(chapter)) current = i;
        int target = current + direction;
        if (target >= 0 && target < chapters.size()) loadChapter(chapters.get(target).id, direction > 0 ? 0 : -2);
        else Toast.makeText(this, direction > 0 ? "已经到最后一章" : "已经到第一章", Toast.LENGTH_SHORT).show();
    }
    private void chooseChapter() {
        String[] titles = new String[chapters.size()]; for (int i = 0; i < titles.length; i++) titles[i] = chapters.get(i).title;
    ...
    private void setChrome(boolean visible) {
        chrome = visible; top.setVisibility(visible ? View.VISIBLE : View.GONE); bottom.setVisibility(visible ? View.VISIBLE : View.GONE);
        getWindow().getDecorView().setSystemUiVisibility(visible ? View.SYSTEM_UI_FLAG_VISIBLE : View.SYSTEM_UI_FLAG_FULLSCREEN | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION | View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY);
    }
    ...
    private final class Holder extends RecyclerView.ViewHolder {
        final FrameLayout frame; final ZoomImageView picture; final Button error; final ProgressBar spinner;
        ReaderImages.Request request; int position; boolean loaded;
        Holder(FrameLayout frame) {
            super(frame); this.frame = frame; picture = new ZoomImageView(ReaderActivity.this);
            frame.addView(picture, new FrameLayout.LayoutParams(-1, -1));
            spinner = new ProgressBar(ReaderActivity.this); frame.addView(spinner, new FrameLayout.LayoutParams(48, 48, Gravity.CENTER));
            error = button("图片读取失败 · 点此重试", v -> bind(position)); error.setVisibility(View.GONE);
            frame.addView(error, new FrameLayout.LayoutParams(-2, -2, Gravity.CENTER));
            picture.setNavigationListener(new ZoomImageView.NavigationListener() {
                public void onPrevious() { flip(mode == 1 ? 1 : -1); }
                public void onNext() { flip(mode == 1 ? -1 : 1); }
                public void onCenterTap() { setChrome(!chrome); }
            }); picture.setPagerManaged(mode != 2);
        }
        void cancel() { if (request != null) request.cancel(); loaded = false; picture.setImageDrawable(null); }
        void bind(int p) {
            cancel(); position = p; error.setVisibility(View.GONE); spinner.setVisibility(View.VISIBLE);
            int chapterGeneration = generation;
            request = images.load(pages.get(p).url, getResources().getDisplayMetrics().widthPixels, new ReaderImages.Callback() {
                public void complete(Bitmap bitmap) {
                    if (!valid(chapterGeneration) || position != p) return;
                    picture.setImageBitmap(bitmap); loaded = true; spinner.setVisibility(View.GONE);
                    if (mode == 2) { int w = Math.max(1, canvas.getWidth()); frame.getLayoutParams().height = Math.max(1, (int)((long)w * bitmap.getHeight() / bitmap.getWidth())); frame.requestLayout(); }
                    if (position == index) save(false);
                }
    ...
    private final class Pages extends RecyclerView.Adapter<Holder> {
        public Holder onCreateViewHolder(ViewGroup parent, int type) {
            FrameLayout frame = new FrameLayout(ReaderActivity.this); frame.setBackgroundColor(Color.BLACK);
            frame.setLayoutParams(new RecyclerView.LayoutParams(-1, mode == 2 ? Math.max(1, canvas.getHeight()) : -1));
            Holder holder = new Holder(frame); holders.add(holder); return holder;
        }
