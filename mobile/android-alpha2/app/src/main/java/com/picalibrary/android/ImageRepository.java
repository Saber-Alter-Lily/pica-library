...
final class ImageRepository {
    ...
    private static final long DISK_LIMIT = 256L * 1024 * 1024;
    ...
    private static String key(String path){
        try{
            byte[] digest=MessageDigest.getInstance("SHA-256").digest(path.getBytes(java.nio.charset.StandardCharsets.UTF_8));StringBuilder b=new StringBuilder();for(byte v:digest)b.append(String.format(java.util.Locale.ROOT,"%02x",v));return b.toString();
        }catch(Exception e){return Integer.toHexString(path.hashCode());}
    }
    private static File dir(Context context){File d=new File(context.getFilesDir(),"desktop-cover-cache-v1");d.mkdirs();return d;}
    ...
    private static synchronized void trim(Context context,File active){File[] files=dir(context).listFiles();if(files==null)return;Arrays.sort(files,Comparator.comparingLong(File::lastModified));long total=0;for(File f:files)total+=f.length();for(File f:files)if(total>DISK_LIMIT&&!f.equals(active)){long n=f.length();if(f.delete())total-=n;}}
    ...
    static void load(Activity activity, ImageView target, String path, int placeholderColor) {
        ...
        if (hit == null) hit=diskHit(activity,path);
        if (hit != null) {
            put(path,hit);target.setImageBitmap(hit);return;
        }
        target.setImageDrawable(new ColorDrawable(placeholderColor));
        POOL.submit(() -> {
            Bitmap bitmap=cacheNow(activity,path);
    ...
    static void prefetch(Activity activity, String path) {
        if (path == null || path.isEmpty() || cached(path) != null || disk(activity,path).isFile()) return;
        POOL.submit(() -> cacheNow(activity,path));
    }
