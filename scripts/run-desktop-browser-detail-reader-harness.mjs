...
async function waitForDesktop(home, options = {}) {
    ...
    throw new Error('Desktop engine did not reach the required J5 state')
}
...
async function waitForExit(child, timeoutMs = 40_000) {
    ...
    return await new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
            child.kill('SIGKILL')
            reject(new Error('Desktop engine did not exit after J5 shutdown'))
        }, timeoutMs)
...
async function postSettings(url, status, libraryDirectory) {
    ...
    if (!response.ok)
        throw new Error(
            `Synthetic J5 Desktop setup failed: HTTP ${response.status}`
        )
...
async function shutdownDesktop(home, child) {
    ...
    try {
        ...
        if (exited.exitCode !== 0)
            throw new Error(
                `Desktop J5 process exited non-zero: ${exited.exitCode} / ${exited.signal}`
            )
    } catch (error) {
        if (child.exitCode === null && child.signalCode === null)
            child.kill('SIGKILL')
...
async function main() {
    ...
    const toolRoot = fs.mkdtempSync(
        path.join(os.tmpdir(), 'pica-j5-playwright-')
    )
    const desktopHome = fs.mkdtempSync(
        path.join(os.tmpdir(), 'pica-j5-desktop-')
    )
    const libraryDirectory = path.join(desktopHome, 'benchmark-library')
    const fixtureFile = path.join(desktopHome, 'j5-fixture.json')
    ...
    try {
        ...
        if (
            ...
            throw new Error(
                `J5 refuses system-level credential backend "${credentialBackend || 'unknown'}"; run on Windows DPAPI with isolated Desktop home or a session-memory backend`
            )
