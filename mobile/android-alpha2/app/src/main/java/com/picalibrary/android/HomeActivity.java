...
public final class HomeActivity extends LocaleAwareActivity {
    ...
    private void showLibraryMenu(){String[] labels={"书架","刷新书库","显示方式"};new AlertDialog.Builder(this).setTitle(LocalizedText.ui("书库")).setItems(LocalizedText.ui(labels),(d,w)->{if(w==0){showingShelves=true;show();}else if(w==1)refreshLibrary(true);else chooseCollectionMode();}).setNegativeButton(LocalizedText.ui("取消"),null).show();}
