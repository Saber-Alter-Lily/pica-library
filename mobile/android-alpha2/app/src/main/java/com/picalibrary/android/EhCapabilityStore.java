...
final class EhCapabilityStore {
    ...
    static Snapshot load(Context context){Context app=context.getApplicationContext();if(!EhClient.accountAvailable(app))return new Snapshot(State.NOT_CONNECTED,0);SharedPreferences p=app.getSharedPreferences(PREF,Context.MODE_PRIVATE);State state=State.UNKNOWN;try{state=State.valueOf(p.getString("exhState",State.UNKNOWN.name()));}catch(Exception ignored){}return new Snapshot(state,p.getLong("checkedAt",0));}

    static synchronized Snapshot refresh(Context context,boolean force){Context app=context.getApplicationContext();if(!EhClient.accountAvailable(app)){clear(app);return new Snapshot(State.NOT_CONNECTED,0);}long now=System.currentTimeMillis();Snapshot prior=load(app);if(!force&&prior.fresh(now))return prior;String raw=new EhClient(app).probeExH();State state="AVAILABLE".equals(raw)?State.AVAILABLE:"NETWORK_ERROR".equals(raw)?State.NETWORK_ERROR:State.CURRENTLY_UNAVAILABLE;Snapshot next=new Snapshot(state,now);save(app,next);return next;}
    ...
    static void invalidate(Context context){Context app=context.getApplicationContext();if(!EhClient.accountAvailable(app)){clear(app);return;}app.getSharedPreferences(PREF,Context.MODE_PRIVATE).edit().putString("exhState",State.UNKNOWN.name()).putLong("checkedAt",0).apply();}
