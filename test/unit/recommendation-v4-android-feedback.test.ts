...
describe('Recommendation V4 Android feedback parity',()=>{
  it('stores sentiment immediately and keeps reasons optional',()=>{
    ...
    expect(method).toContain('sentiment.equals(RecommendationFeedbackStore.sentiment(this,item.comicId))')
  ...
  it('keeps the presented batch stable while feedback affects later recommendation cycles',()=>{
    ...
    expect(method).not.toContain('if(!RecommendationFeedbackStore.askReasons(this)){show();return;}')
    expect(method).not.toContain('(d,w)->show()')
    expect(method).not.toContain('.setOnCancelListener(d->show())')
