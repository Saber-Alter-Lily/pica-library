...
final class EhClient {
    interface FavoriteSyncControl { void checkpoint(String phase,int done,int total) throws Exception; }
    ...
    private List<Comic> gdata(List<String[]> refs,String surface) throws Exception {return gdata(refs,surface,null);}
    private List<Comic> gdata(List<String[]> refs,String surface,FavoriteSyncControl control) throws Exception {List<Comic> out=new ArrayList<>();int batch=0;for(int offset=0;offset<refs.size();offset+=25){if(control!=null)control.checkpoint("正在读取 E-H 元数据",Math.min(offset,refs.size()),refs.size());JSONArray ids=new JSONArray();for(int i=offset;i<Math.min(offset+25,refs.size());i++){String[] ref=refs.get(i);JSONArray pair=new JSONArray();pair.put(Long.parseLong(ref[0]));pair.put(ref[1]);ids.put(pair);}JSONObject request=new JSONObject();request.put("method","gdata");request.put("gidlist",ids);request.put("namespace",1);JSONArray rows=postJson(request).optJSONArray("gmetadata");if(rows==null)throw new IOException("E-H 元数据响应格式异常");for(int i=0;i<rows.length();i++){JSONObject row=rows.optJSONObject(i);if(row!=null&&!row.has("error"))out.add(parseComic(row,surface));}batch++;if(control!=null)control.checkpoint("正在读取 E-H 元数据",Math.min(offset+25,refs.size()),refs.size());if(offset+25<refs.size())Thread.sleep(batch%4==0?5000:250);}return out;}
    ...
    EhFavoriteSync favoritesSnapshot() throws Exception {return favoritesSnapshot(null);}
    EhFavoriteSync favoritesSnapshot(FavoriteSyncControl control) throws Exception {
        if(control!=null)control.checkpoint("正在读取 E-H 收藏",0,1);
        ...
        if(control!=null)control.checkpoint("正在验证 E-H 收藏",0,1);verifyAccount();if(control!=null)control.checkpoint("正在读取 E-H 收藏分类",0,1);List<String> categoryNames=parseFavoriteCategoryNames(accountText(ORIGIN+"/uconfig.php"));LinkedHashMap<String,String[]> refs=new LinkedHashMap<>();LinkedHashMap<String,FavoriteMeta> metaById=new LinkedHashMap<>();Set<String> cursors=new HashSet<>();String url=ORIGIN+"/favorites.php?favcat=all";Pattern next=Pattern.compile("(?:\\?|&amp;|&)next=(\\d+)");
        for(int page=0;page<200;page++){if(control!=null)control.checkpoint("正在读取 E-H 收藏页",page,200);String html=accountText(url);refs.putAll(extractGalleryRefs(html,0));parseFavoriteRows(html,categoryNames,metaById);Matcher nm=next.matcher(html);String cursor="";if(nm.find())cursor=nm.group(1);if(cursor.isEmpty()||!cursors.add(cursor))break;url=ORIGIN+"/favorites.php?favcat=all&next="+URLEncoder.encode(cursor,"UTF-8");Thread.sleep(250);}
        if(control!=null)control.checkpoint("正在读取 E-H 元数据",0,refs.size());List<Comic> comics=refs.isEmpty()?Collections.emptyList():gdata(new ArrayList<>(refs.values()),"eh",control);ArrayList<EhFavoriteSync.Item> items=new ArrayList<>();int[] counts=new int[10];for(Comic comic:comics){FavoriteMeta meta=metaById.get(comic.id);int slot=meta==null?-1:meta.slot;String note=meta==null?"":meta.note;items.add(new EhFavoriteSync.Item(comic.id,slot,note));if(slot>=0&&slot<10)counts[slot]++;}return new EhFavoriteSync(comics,items,categoryNames,counts);
    }
