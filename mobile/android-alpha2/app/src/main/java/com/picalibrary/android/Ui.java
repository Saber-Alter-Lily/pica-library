...
final class Ui {
    ...
    static TextView text(Context c,String s,float sp,int color,boolean bold){return textInternal(c,LocalizedText.ui(c,s),sp,color,bold);}
    static TextView rawText(Context c,String s,float sp,int color,boolean bold){return textInternal(c,s,sp,color,bold);}
    private static TextView textInternal(Context c,String s,float sp,int color,boolean bold){TextView v=new TextView(c);v.setText(s);v.setTextSize(sp);v.setTextColor(color);v.setTypeface(Typeface.DEFAULT,bold?Typeface.BOLD:Typeface.NORMAL);v.setLineSpacing(0,1.08f);return v;}
    ...
    static Button infoButton(Context c,String title,String message){
        String localizedTitle=LocalizedText.ui(c,title), localizedMessage=LocalizedText.ui(c,message);
        Button b=new Button(c);b.setText("!");b.setAllCaps(false);b.setTextSize(10f);b.setTextColor(MUTED);b.setMinWidth(0);b.setMinimumWidth(0);b.setMinHeight(0);b.setMinimumHeight(0);int size=dp(c,24);b.setLayoutParams(new LinearLayout.LayoutParams(size,size));b.setPadding(0,0,0,0);b.setGravity(Gravity.CENTER);b.setBackground(outlined(SURFACE,OUTLINE,12,c));b.setContentDescription(c.getString(R.string.info_description,localizedTitle));b.setOnClickListener(v->new AlertDialog.Builder(c).setTitle(localizedTitle).setMessage(localizedMessage).setPositiveButton(R.string.common_ok,null).show());return b;
    }
