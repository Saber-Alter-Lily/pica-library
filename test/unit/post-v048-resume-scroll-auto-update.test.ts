...
describe('post-v0.4.8 mobile resume, preference scroll and update notices', () => {
    it('keeps Android recommendation work alive while user-paused', () => {
        ...
        expect(jobs).toContain('static void pause(Context context)')
        expect(jobs).not.toContain('setPaused(context,"recommendation",UNIQUE_NAME,true);WorkManager.getInstance')
        expect(jobs).toContain('pauseAcknowledged(Context context)')
        expect(jobs).toContain('acknowledgePause(Context context,boolean value)')
    ...
    it('checks and prompts for updates automatically on Desktop without auto-installing', () => {
        ...
        expect(app).toContain('AUTO_UPDATE_CHECK_INTERVAL_MS = 6 * 60 * 60 * 1000')
        expect(app).toContain('automaticUpdateCheckDue()')
        expect(app).toContain('writeAutoUpdatePromptState({ checkedAt: Date.now() })')
