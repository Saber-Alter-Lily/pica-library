...
- **完整程序包更新**：网页会显示准确的 Windows ZIP 与替换说明；若 Release 提供“升级助手”，优先使用升级助手。v0.4.0 → v0.4.1 属于这一类。
- **v0.4.0 → v0.4.1 推荐路径**：从 v0.4.1 Release 下载 `Pica-Library-v0.4.1-upgrade-assistant.zip`，解压后双击 `Upgrade-Pica-Library-v0.4.1.cmd`。助手会自动下载并校验官方完整包、识别旧程序目录、关闭旧版、建立程序与数据库安全快照、替换程序并执行健康检查；失败时自动回滚。
