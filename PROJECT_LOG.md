...
- P3A-2 增加 Channel Query Compiler：把抽象召回通道编译为 Pica / E-H / ExH 原生查询计划，作者/IP 在 E-H 分别使用 `artist:` / `parody:` 精确 namespace，Pica 保留标签/分类/关键词语义，探索映射为 Pica loved / E-H popular，RELATED 仅编译 Pica native related，REDISCOVERY 保持本地；当前 `mode=PLAN_ONLY`、`executionEnabled=false`、`persistenceEnabled=false`，不发请求、不落库。
