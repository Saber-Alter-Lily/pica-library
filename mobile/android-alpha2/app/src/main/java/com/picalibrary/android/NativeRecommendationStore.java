...
final class NativeRecommendationStore {
    ...
    static synchronized void markCurrentSeen(Context context){Snapshot s=load(context);if(!s.available())return;for(Item item:s.current())RecommendationEvidenceStore.recordImpression(context,item);markCurrent(s);save(context,s);}
    static synchronized void markSeen(Context context,Collection<Item> items){Snapshot s=load(context);if(items!=null)for(Item item:items)if(item!=null&&!item.comicId.isEmpty()){RecommendationEvidenceStore.recordImpression(context,item);s.displayedIds.add(item.comicId);}save(context,s);}
