...
public final class EhAccountActivity extends Activity {
    ...
    private void showAccountActions(){String[] labels={"更新网页登录","手动更新会话","重新检查 ExH","清除本机会话"};new AlertDialog.Builder(this).setTitle("账号管理").setItems(labels,(d,w)->{if(w==0)startActivity(new Intent(this,EhWebLoginActivity.class));else if(w==1)startActivity(new Intent(this,EhSessionActivity.class));else if(w==2)probe();else confirmClearSession();}).setNegativeButton("取消",null).show();}
    ...
    private void probe(){if(!EhAccountStore.load(this).configured()){if(exhState!=null){exhState.setText("需 E-H 登录");exhState.setTextColor(Ui.MUTED);}return;}final int id=++probeSerial;if(exhState!=null){exhState.setText("检查中");exhState.setTextColor(Ui.MUTED);}new Thread(()->{String value=new EhClient(this).probeExH();runOnUiThread(()->{if(id!=probeSerial||isDestroyed()||exhState==null)return;String label="AVAILABLE".equals(value)?"可用":"NETWORK_ERROR".equals(value)?"网络异常":"当前不可访问";exhState.setText(label);exhState.setTextColor(SettingsRow.statusColor(label));});}).start();}
