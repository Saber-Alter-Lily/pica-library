# Desktop Active-download Foreground Latency — P2 J7
...
J7 adds one deterministic load source so the download-active foreground path is
repeatable in CI and on developer machines without Provider credentials or
external network variability.

J7 does not replace J1/J2. It creates the controlled workload and delegates all
latency/window measurement to J2.

## Controlled workload

J7 uses the real production:
...
J7 requires **two independent signals** during the measured foreground window.
...
J7 imports `runHttpLatencyScenario()` and never reimplements:
...
J7 is intentionally a controlled local-load benchmark.
...
2. J7 imports J2 rather than cloning its measurement logic;
