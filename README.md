...
- **长任务不再黑盒运行**：推荐生成、画风分析、收藏同步与 WebDAV 同步统一补齐阶段、进度、暂停 / 继续、取消与失败反馈；切换页面不影响后端权威任务状态。
- **修复推荐网络卡死与重启后仍卡住**：Pica 普通 API 增加 15 秒超时；连续 Provider 失败会快速止损；程序启动自动清理上次进程中断遗留的 `buildingCycleId`，不再需要删除 `library.db`。
- **失败刷新保留上一轮可用推荐**：候选不足或网络失败的新周期不会替换旧周期；Desktop 与 Android 都只在完整生成成功后提交新结果。
- **画风任务可控且有上限**：Desktop 画风索引支持暂停 / 继续 / 取消；模型加载最长 120 秒、单页分析最长 45 秒，未完成作品保持待分析，之后可以继续。
- **收藏与 WebDAV 同步可暂停恢复**：收藏同步按页 checkpoint；WebDAV 按漫画 / 章节 / 页面 checkpoint，取消后已上传的 SHA 匹配对象会在下一次同步复用；本地 WebDAV 扫描改为异步文件读取，避免长时间占住 Node 事件循环。
- **Android 后台任务统一收口**：推荐、Pica 收藏同步、电脑收藏 / 封面导入和下载任务都进入统一任务中心；支持暂停 / 继续 / 取消 / 重试。下载改为逐页持久化，暂停后从已完成页面真正续传；推荐暂停后重新开始当前轮，但上一轮推荐始终保留。
- **公开升级路径**：Windows v0.4.0 使用 v0.4.7 升级助手一次直达；v0.4.1–v0.4.6 可使用兼容增量更新。Android 正式版为 v50 / 0.4.7。
...
- 面向普通用户的公开升级基线是 v0.4.0。Windows v0.4.0 使用 v0.4.7 Release 中的 **Windows 升级助手** 一次直达最新版，不需要安装任何中间版本。
...
已有 **v0.4.0** 的用户直接下载 `Pica-Library-v0.4.7-upgrade-assistant.zip`，解压后双击 `Upgrade-Pica-Library-v0.4.7.cmd`，即可一次升级到当前最新版；无需安装任何中间版本。助手会保护 `%LOCALAPPDATA%\Pica Library`，并自动完成校验、备份、替换、健康检查与失败回滚。

### Android

Android APK 不通过应用商店分发，请只使用本仓库正式 Release。已有 **v42 / 0.4.0** 的用户可直接通过应用内更新链原地升级到 **v50 / 0.4.7**；无需安装任何中间版本。
