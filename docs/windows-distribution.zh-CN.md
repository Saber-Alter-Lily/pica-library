...
1. 从项目的 [GitHub Releases](https://github.com/Saber-Alter-Lily/pica-library/releases/latest) 下载当前稳定版 Windows 完整包；v0.4.2 对应 `Pica-Library-v0.4.2-windows-x64.zip`。
...
- **兼容增量更新**：v0.4.1 → v0.4.2 可在软件内直接下载、校验、重启并完成更新。
- **完整程序包更新**：仍停留在 v0.4.0 时，从 v0.4.2 Release 下载 `Pica-Library-v0.4.2-upgrade-assistant.zip`，解压后双击 `Upgrade-Pica-Library-v0.4.2.cmd` 直接跨代升级。助手会自动下载并校验官方完整包、识别旧程序目录、关闭旧版、建立程序与数据库安全快照、替换程序并执行健康检查；失败时自动回滚。
...
```powershell
Get-FileHash .\Pica-Library-v0.4.2-windows-x64.zip -Algorithm SHA256
```
