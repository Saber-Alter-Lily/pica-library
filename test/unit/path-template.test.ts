...
import {
    ...
    renderLibraryPath,
    safePathSegment
} from '../../src/library/path-template'
...
describe('library path templates', () => {
    ...
    it('renders the default author-first layout', () => {
        expect(
            renderLibraryPath('library', defaultLibraryTemplate, values)
        ).toBe(
    ...
    it('sanitizes reserved Windows names and rejects unknown fields', () => {
        ...
        expect(() => renderLibraryPath('library', '{unknown}', values)).toThrow(
            'Unknown path placeholder'
        )
    })

    it('supports optional metadata fields and resolves preview collisions', () => {
        const first = renderLibraryPath(
            'library',
            '{circle}/{category}/{title}',
            { ...values, circle: 'Group', category: 'Drama' }
        )
