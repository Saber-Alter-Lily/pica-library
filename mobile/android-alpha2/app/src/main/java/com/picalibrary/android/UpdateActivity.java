...
public final class UpdateActivity extends LocaleAwareActivity {
    ...
    private void check(){
        stopPolling();loading.setVisibility(View.VISIBLE);loading.setIndeterminate(true);action.setEnabled(false);cancel.setVisibility(View.GONE);status.setText(LocalizedText.ui("正在检查更新…"));detail.setText("");
        worker.submit(()->{try{UpdateClient.Info value=UpdateClient.check(this);runOnUiThread(()->{if(!destroyed)show(value);});}catch(Exception e){runOnUiThread(()->{if(destroyed)return;loading.setVisibility(View.GONE);status.setText(LocalizedText.ui("检查更新失败"));detail.setText(LocalizedText.ui("请检查网络后重试"));retryCheckButton();});}});
    }
    ...
    private void show(UpdateClient.Info value){
        ...
        if(!UpdateClient.newer(this,value)){status.setText(LocalizedText.ui("当前已是最新版本"));detail.setText(value.versionName);retryCheckButton();return;}
        status.setText(LocalizedText.ui("发现新版本 · ")+value.versionName);detail.setText(LocalizedText.ui("准备下载更新"));
    ...
    private void retryCheckButton(){action.setText(LocalizedText.ui("重新检查"));action.setEnabled(true);action.setOnClickListener(v->check());cancel.setVisibility(View.GONE);}
    ...
    private void download(){
        ...
        try{
            DownloadManager.Request request=new DownloadManager.Request(Uri.parse(info.apkUrl));request.setTitle(LocalizedText.ui("Pica Library 更新 ")+info.versionName);request.setDescription("正在下载更新");request.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);request.setAllowedOverMetered(true);request.setAllowedOverRoaming(false);request.setDestinationUri(Uri.fromFile(file));
            DownloadManager manager=(DownloadManager)getSystemService(DOWNLOAD_SERVICE);downloadId=manager.enqueue(request);lastBytes=-1;lastProgressAt=System.currentTimeMillis();prefs().edit().putString(P_SHA,info.sha256).putInt(P_CODE,info.versionCode).putString(P_NAME,info.versionName).putLong(P_DOWNLOAD,downloadId).putLong(P_LAST_BYTES,lastBytes).putLong(P_LAST_PROGRESS,lastProgressAt).apply();status.setText(LocalizedText.ui("正在下载更新"));detail.setText(LocalizedText.ui("等待系统下载服务…"));action.setText(LocalizedText.ui("下载中"));action.setEnabled(false);cancel.setText(LocalizedText.ui("取消下载"));cancel.setVisibility(View.VISIBLE);startPolling();
        }catch(Exception e){clearDownloadState(false);status.setText(LocalizedText.ui("无法启动下载"));detail.setText(e.getMessage()==null?LocalizedText.ui("请稍后重试"):e.getMessage());readyDownloadButton("重新下载");}
    }
    ...
    private void updateDownloadState(){
        ...
        if(!d.found){stopPolling();clearDownloadState(false);status.setText(LocalizedText.ui("下载任务已失效"));detail.setText(LocalizedText.ui("系统已找不到原下载任务，可以重新开始"));readyDownloadButton("重新下载");return;}
        if(d.status==DownloadManager.STATUS_SUCCESSFUL){stopPolling();verifyDownloaded();return;}
        if(d.status==DownloadManager.STATUS_FAILED){stopPolling();String reason=reasonText(d.status,d.reason);clearDownloadState(true);status.setText(LocalizedText.ui("下载失败"));detail.setText(reason);readyDownloadButton("重新下载");return;}
        ...
        if(d.total>0){loading.setIndeterminate(false);int pct=(int)Math.max(0,Math.min(100,(d.bytes*100L)/d.total));loading.setProgress(pct);detail.setText(formatBytes(d.bytes)+" / "+formatBytes(d.total)+" · "+pct+"%");}else{loading.setIndeterminate(true);detail.setText(d.bytes>0?LocalizedText.ui("已下载 ")+formatBytes(d.bytes):reasonText(d.status,d.reason));}
        if(stalled){status.setText(LocalizedText.ui("下载长时间没有进度"));detail.setText((d.total>0?detail.getText()+" · ":"")+LocalizedText.ui("可以终止错误任务并重新开始"));action.setText(LocalizedText.ui("终止并重新开始"));action.setEnabled(true);action.setOnClickListener(v->cancelCurrent(true));cancel.setText(LocalizedText.ui("取消下载"));return;}
        if(d.status==DownloadManager.STATUS_PENDING){status.setText(LocalizedText.ui("等待开始下载"));if(d.bytes==0)detail.setText(reasonText(d.status,d.reason));}
        else if(d.status==DownloadManager.STATUS_PAUSED){status.setText(LocalizedText.ui("下载已暂停"));detail.setText(reasonText(d.status,d.reason));action.setText(LocalizedText.ui("终止并重新开始"));action.setEnabled(true);action.setOnClickListener(v->cancelCurrent(true));}
        else status.setText(LocalizedText.ui("正在下载更新"));
        if(d.status!=DownloadManager.STATUS_PAUSED){action.setText(LocalizedText.ui("下载中"));action.setEnabled(false);}cancel.setText(LocalizedText.ui("取消下载"));
    }
    ...
    private void cancelCurrent(boolean restart){
        long id=downloadId;if(id>=0)try{((DownloadManager)getSystemService(DOWNLOAD_SERVICE)).remove(id);}catch(Exception ignored){}stopPolling();clearDownloadState(true);status.setText(restart?LocalizedText.ui("正在重新开始…"):LocalizedText.ui("下载已取消"));detail.setText("");if(restart&&info!=null)handler.postDelayed(this::download,250);else readyDownloadButton("重新下载");
    }
    ...
    private void verifyDownloaded(){
        File file=target();String sha=prefs().getString(P_SHA,"");int expectedCode=prefs().getInt(P_CODE,-1);if(!file.isFile()||sha.isEmpty()){clearDownloadState(false);readyDownloadButton("重新下载");return;}loading.setVisibility(View.VISIBLE);loading.setIndeterminate(true);status.setText(LocalizedText.ui("下载完成，正在验证更新包…"));detail.setText(LocalizedText.ui("正在校验 SHA-256、包名、版本号和签名"));cancel.setVisibility(View.GONE);action.setEnabled(false);
        worker.submit(()->{try{if(!UpdateClient.verify(file,sha))throw new IllegalStateException("SHA-256 mismatch");UpdateClient.verifyPackage(getApplicationContext(),file,expectedCode);runOnUiThread(()->{if(destroyed)return;loading.setVisibility(View.GONE);loading.setIndeterminate(false);loading.setProgress(100);clearDownloadState(false);status.setText(LocalizedText.ui("更新已准备好"));detail.setText(LocalizedText.ui("验证通过，可以安装"));action.setText(LocalizedText.ui("安装更新"));action.setEnabled(true);action.setOnClickListener(v->install(file));});}catch(Exception e){runOnUiThread(()->{if(destroyed)return;file.delete();clearDownloadState(false);loading.setVisibility(View.GONE);status.setText(LocalizedText.ui("更新包验证失败"));detail.setText(LocalizedText.ui("已删除异常文件，请重新下载"));readyDownloadButton("重新下载");});}});
    }

    private void install(File file){if(Build.VERSION.SDK_INT>=26&&!getPackageManager().canRequestPackageInstalls()){new AlertDialog.Builder(this).setTitle(LocalizedText.ui("允许安装更新")).setMessage(LocalizedText.ui("请允许 Pica Library 安装下载的更新。返回后再次点击“安装更新”即可。")) .setNegativeButton(LocalizedText.ui("取消"),null).setPositiveButton(LocalizedText.ui("打开设置"),(d,w)->startActivity(new Intent(Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES,Uri.parse("package:"+getPackageName())))).show();return;}Uri uri=FileProvider.getUriForFile(this,getPackageName()+".files",file);Intent intent=new Intent(Intent.ACTION_VIEW);intent.setDataAndType(uri,"application/vnd.android.package-archive");intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION|Intent.FLAG_ACTIVITY_NEW_TASK);startActivity(intent);}
