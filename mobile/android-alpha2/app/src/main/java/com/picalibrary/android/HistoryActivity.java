...
public final class HistoryActivity extends LocaleAwareActivity {
    ...
    private List<ComicHistory> grouped(List<ReadingHistoryStore.Session> rows){LinkedHashMap<String,ComicHistory> map=new LinkedHashMap<>();for(ReadingHistoryStore.Session row:rows){ComicHistory group=map.get(row.comicId);if(group==null){group=new ComicHistory(row.comicId);group.latest=row;map.put(row.comicId,group);}group.sessions.add(row);}return new ArrayList<>(map.values());}
    private boolean genericChapterTitle(String value){return value==null||value.trim().isEmpty()||"章节".equals(value.trim());}
