...
describe('account onboarding distribution candidate', () => {
    it('uses distinct monotonically increasing desktop and Android versions', () => {
        ...
        expect(gradle).toContain(
            "System.getenv('PICA_ANDROID_VERSION_CODE') ?: '43'"
        )
    ...
    it('requires the checksum-verified official v0.4.0 base without replacing the launcher', () => {
        const script = fs.readFileSync(
            'scripts/build-windows-package.ps1',
            'utf8'
        )
        expect(script).toContain("$version -eq '0.4.1'")
        expect(script).toContain(
            'artifacts\\release-base\\Pica-Library-v0.4.0-windows-x64.zip'
        )
        expect(script).toContain(
            '1e22df88d067c34152e1a94008fbb4c378c543fc41450768e01c25c105e698d0'
        )
        expect(script).toContain(
