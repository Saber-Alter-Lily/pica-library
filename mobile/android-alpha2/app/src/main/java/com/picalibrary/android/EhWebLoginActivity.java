...
public final class EhWebLoginActivity extends Activity {
    ...
    private static final String[] COOKIE_URLS={"https://forums.e-hentai.org/","https://e-hentai.org/","https://exhentai.org/"};
    ...
    private void capture(boolean force){
        if(verifying||destroyed)return;Map<String,String> jar=new LinkedHashMap<>();CookieManager manager=CookieManager.getInstance();for(String url:COOKIE_URLS)mergeCookies(jar,manager.getCookie(url));String member=clean(jar.get("ipb_member_id")),hash=clean(jar.get("ipb_pass_hash"));if(member.isEmpty()||hash.isEmpty()){if(force)showStatus("尚未检测到登录会话",Ui.BAD);return;}String signature=member+"|"+hash+"|"+clean(jar.get("igneous"))+"|"+clean(jar.get("cf_clearance"));if(!force&&signature.equals(lastSignature))return;lastSignature=signature;EhAccountStore.Session old=EhAccountStore.load(this);boolean sameAccount=member.equals(old.memberId);String ign=clean(jar.get("igneous"));if(ign.isEmpty()&&sameAccount)ign=old.igneous;String cf=clean(jar.get("cf_clearance"));if(cf.isEmpty()&&sameAccount)cf=old.cfClearance;EhAccountStore.Session next=new EhAccountStore.Session(member,hash,ign,cf);verify(old,next);
    }

    private void verify(EhAccountStore.Session old,EhAccountStore.Session next){
        verifying=true;showStatus("正在连接…",Ui.MUTED);new Thread(()->{try{EhAccountStore.save(this,next);new EhClient(this).verifyAccount();runOnUiThread(()->{if(destroyed)return;verifying=false;Toast.makeText(this,"E-H 账号已登录",Toast.LENGTH_SHORT).show();setResult(RESULT_OK);finish();});}catch(Exception e){try{if(old.configured())EhAccountStore.save(this,old);else EhAccountStore.clear(this);}catch(Exception ignored){}runOnUiThread(()->{if(destroyed)return;verifying=false;showStatus(errorMessage(e),Ui.BAD);});}}).start();
    }
