...
/** Public E-H online reader source. Stable image-page URLs are resolved only when the page bitmap is requested. */
final class EhReaderSource implements ReaderSource {
    ...
    public BridgeClient.ChapterData chapter(String comicId,String episodeId) throws Exception {
        ...
        BridgeClient.ChapterItem known=knownChapters.get(episodeId);int expected=known==null?0:Math.max(0,known.downloadedPictures);List<EhClient.Page> remote=client.pages(comicId,expected);if(remote.isEmpty())throw new IllegalStateException("E-H 画廊没有可读取页面");
        List<BridgeClient.PageItem> pages=new ArrayList<>();for(EhClient.Page page:remote)pages.add(new BridgeClient.PageItem(page.id,page.pageUrl,page.position));BridgeClient.ChapterItem item=new BridgeClient.ChapterItem(episode.id,episode.title,episode.order,pages.size());knownChapters.put(item.id,item);return new BridgeClient.ChapterData(item,pages,0);
    }
