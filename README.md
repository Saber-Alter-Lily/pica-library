...
- **在线 → 来源**：新增 E-Hentai；ExHentai 保留为 E-H 账号下的可选扩展。
- **在线 → E-H 浏览**：支持最新、热门、收藏、关注、分类、排行榜和高级筛选。
- **标签**：接入 EhTagTranslation 中文显示；底层仍保留 E-H canonical tag。
- **推荐**：Pica 与 E-H 收藏共同构成兴趣画像；ExH 可用时只扩充候选池。
- **漫画详情 → 作者**：新增归一作者目录和跨来源作者作品页。
- **书库 → 历史**：恢复阅读历史；支持时间段、指定日期和继续阅读。
- **书库 → 筛选**：存储位置与内容来源分离；排序独立，常用筛选前置。
- **Desktop / Web**：同步 Android 新增的多来源、作者、历史和 E-H 交互逻辑。
