...
final class ReaderImages implements AutoCloseable {
    ...
    ReaderImages(Context context, ReaderSource source) {
        ...
        cache = MobileStoragePaths.cacheDir(this.context, ROOT + "/" + source.scope());
    }
    ...
    Request load(String path, int width, Callback callback) {
        ...
        request.future = workers.submit(() -> {
            ...
            try {
                if (!target.isFile()) {
                    partial = fetchToPartial(path, request);
                    validateImage(partial);
                    synchronized (ReaderImages.class) { if (!target.exists() && !partial.renameTo(target)) throw new IOException("cache rename failed"); }
                }
                if (request.cancelled || closed) return;
                BitmapFactory.Options options = new BitmapFactory.Options(); options.inJustDecodeBounds = true;BitmapFactory.decodeFile(target.getPath(), options);
                options.inSampleSize = ReaderPolicy.sampleSize(options.outWidth, options.outHeight, width);options.inJustDecodeBounds = false;
                Bitmap bitmap = BitmapFactory.decodeFile(target.getPath(), options);if (bitmap == null) { target.delete(); throw new IOException("decode failed"); }
    ...
    /** Replaces stale low-priority prefetch targets whenever the visible page changes. */
    void prefetch(List<String> paths) {
        final int generation=prefetchGeneration.incrementAndGet();
        for(String path:paths){
            if(path==null||path.isEmpty())continue;
            File target=new File(cache,ReaderPolicy.hash(path));if(target.isFile())continue;
            prefetchers.submit(()->{
                if(closed||generation!=prefetchGeneration.get())return;
                Request request=new Request();File partial=null;
                try{
                    partial=fetchToPartial(path,request);if(closed||generation!=prefetchGeneration.get())return;validateImage(partial);
                    synchronized(ReaderImages.class){if(!target.exists()&&!partial.renameTo(target))throw new IOException("cache rename failed");}
                    target.setLastModified(System.currentTimeMillis());trim(target);
                }catch(Exception ignored){}finally{if(partial!=null)partial.delete();}
            });
        }
    }

    private File fetchToPartial(String path,Request request) throws Exception {
        File partial=File.createTempFile("page-",".part",cache);HttpURLConnection connection=source.image(path);request.connection=connection;
        try{
            if(request.cancelled||closed)throw new IOException("cancelled");int status=connection.getResponseCode();if(status!=200)throw new IOException("HTTP "+status);
            try(InputStream in=connection.getInputStream();OutputStream out=new FileOutputStream(partial)){byte[] buffer=new byte[32768];int n;long bytes=0;while((n=in.read(buffer))!=-1){if(request.cancelled||closed||Thread.currentThread().isInterrupted())throw new IOException("cancelled");bytes+=n;if(bytes>64L*1024*1024)throw new IOException("page too large");out.write(buffer,0,n);}}
            return partial;
        }catch(Exception e){partial.delete();throw e;}finally{connection.disconnect();request.connection=null;}
    }
    ...
    private synchronized void trim(File active) {
        File root=cache.getParentFile();File[] scopes=root==null?null:root.listFiles(File::isDirectory);if(scopes==null)return;
        ArrayList<File> files=new ArrayList<>();for(File scope:scopes){File[] nested=scope.listFiles(file -> file.isFile()&&!file.getName().endsWith(".part"));if(nested!=null)files.addAll(Arrays.asList(nested));}
        files.sort(Comparator.comparingLong(File::lastModified));long total=0;for(File file:files)total+=file.length();long limit=StorageSettings.pageLimitBytes(context);
        for(File file:files)if(limit!=Long.MAX_VALUE&&total>limit&&!file.equals(active)){long n=file.length();if(file.delete())total-=n;}
    }
