...
final class EhFavoriteStore {
    ...
    static final class Snapshot {
        ...
        String noteOf(String id){RemoteFavorite row=remote.get(id);return row==null?"":row.note;}
    ...
    private static void recountIfNeeded(Snapshot s){boolean any=false;for(int c:s.categoryCounts)if(c>0){any=true;break;}if(any)return;recount(s);}
    private static void recount(Snapshot s){Arrays.fill(s.categoryCounts,0);for(RemoteFavorite row:s.remote.values())if(row.slot>=0&&row.slot<10)s.categoryCounts[row.slot]++;}
    ...
    static synchronized void setRemoteFavorite(Context context,String id,boolean desired){if(desired){Snapshot s=load(context);RemoteFavorite prior=s.remote.get(id);setRemoteFavorite(context,id,prior==null||prior.slot<0?0:prior.slot,prior==null?"":prior.note);}else setRemoteFavorite(context,id,-1,"");}
    static synchronized void setRemoteFavorite(Context context,String id,int slot,String note){if(!EhClient.isEhId(id))throw new IllegalArgumentException("无效的 E-H 收藏标识");if(slot<-1||slot>9)throw new IllegalArgumentException("无效的 E-H 收藏分类");Snapshot s=load(context);if(slot<0){s.remoteIds.remove(id);s.remote.remove(id);}else{s.remoteIds.add(id);s.remote.put(id,new RemoteFavorite(id,slot,note));}recount(s);save(context,s);UnifiedCatalogStore.reconcileLocalReferences(context);NativeRecommendationStore.markFavoriteChange(context);}
    static synchronized void replaceRemote(Context context,List<EhClient.Comic> comics){Snapshot s=load(context);LinkedHashMap<String,RemoteFavorite> prior=new LinkedHashMap<>(s.remote);s.remoteIds.clear();s.remote.clear();for(EhClient.Comic comic:comics)if(comic!=null&&EhClient.isEhId(comic.id)){s.remoteIds.add(comic.id);s.remote.put(comic.id,prior.getOrDefault(comic.id,new RemoteFavorite(comic.id)));}recount(s);save(context,s);UnifiedCatalogStore.reconcileLocalReferences(context);NativeRecommendationStore.markFavoriteChange(context);}
