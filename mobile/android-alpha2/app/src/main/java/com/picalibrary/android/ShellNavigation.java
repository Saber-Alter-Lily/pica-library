...
final class ShellNavigation {
    ...
    private static void navigate(Activity activity,int selected,int target){if(target==selected)return;Intent i;if(target<=1){i=new Intent(activity,HomeActivity.class);i.putExtra("tab",target);i.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP|Intent.FLAG_ACTIVITY_SINGLE_TOP);}else if(target==2)i=new Intent(activity,PicaBrowseActivity.class);else i=new Intent(activity,SettingsActivity.class);activity.startActivity(i);if(!(activity instanceof HomeActivity))activity.finish();}
}
