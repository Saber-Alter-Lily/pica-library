...
final class UnifiedCatalogStore {
    ...
    static Snapshot refreshDesktop(Context context) throws Exception {
        ...
        while(offset<total&&offset<20000){
            JSONObject root=new JSONObject(BridgeClient.get(context,"/mobile/v1/library?scope=library&limit="+pageSize+"&offset="+offset+"&sort=latest"));JSONArray arr=root.optJSONArray("items");int count=arr==null?0:arr.length();total=Math.max(0,root.optInt("total",offset+count));
    ...
    static Snapshot reconcileLocalReferences(Context context){Snapshot snapshot=load(context);applyLocalReferences(context,snapshot);save(context,snapshot);return snapshot;}

    private static void applyLocalReferences(Context context,Snapshot snapshot){
        ...
        if(!favorites.updatedAt.isEmpty()){
            for(Entry entry:snapshot.byId.values())entry.favorite=false;
            for(BridgeClient.ComicItem item:favorites.items){Entry entry=entry(snapshot,item.id);entry.favorite=true;basic(entry,item.title,item.author);entry.desktopCoverPath=item.coverPath;entry.desktopDownloadedPictures=Math.max(entry.desktopDownloadedPictures,item.downloadedPictures);}
        }
    }
    ...
    private static void mergeRemote(Snapshot snapshot,JSONObject o){
        String id=o.optString("comicId","");if(id.isEmpty())return;Entry entry=entry(snapshot,id);basic(entry,o.optString("title",entry.title),o.optString("author",entry.author));entry.canonicalAuthor=o.optString("canonicalAuthor",entry.canonicalAuthor);entry.authorId=o.optString("authorId",entry.authorId);copyIfPresent(o,"tags",entry.tags);copyIfPresent(o,"categories",entry.categories);if(o.has("finished"))entry.finished=o.optBoolean("finished",entry.finished);if(o.has("isFavorite"))entry.favorite=o.optBoolean("isFavorite",entry.favorite);entry.updatedAt=newer(entry.updatedAt,o.optString("updatedAt",""));entry.knownPictures=Math.max(entry.knownPictures,o.optInt("knownPictures",o.optInt("pageCount",0)));entry.remotePageCount=Math.max(0,o.optInt("pageCount",0));entry.remoteEpisodeCount=Math.max(0,o.optInt("episodeCount",0));entry.remoteAvailable=true;entry.remoteCoverPath=o.optString("coverPath","");entry.remoteManifestPath=o.optString("manifestPath","");
    }
