...
if [[ -z "$BASELINE_ARCHIVE" || -z "$CANDIDATE_ARCHIVE" || ! -f "$BASELINE_ARCHIVE" || ! -f "$CANDIDATE_ARCHIVE" ]]; then
  echo "Usage: $0 <baseline-macos-preview.tar.gz> <candidate-macos-preview.tar.gz>" >&2
  exit 2
fi
if [[ "$(uname -s)" != "Darwin" || "$(uname -m)" != "arm64" ]]; then
  echo "macOS preview replacement gate requires an Apple Silicon macOS runner" >&2
  exit 1
fi
...
fail() {
  echo "macOS preview replacement gate: $*" >&2
...
extract_package() {
  ...
  local root
  root="$(find "$destination" -mindepth 1 -maxdepth 1 -type d -name 'Pica-Library-*-macos-arm64-experimental' -print -quit)"
...
if (mode === 'query') {
  ...
  assert(value.items.some((item) => item.comicId === 'macos-replace-1'), 'baseline shelf membership missing')
} else if (mode === 'downloads') {
  assert(Array.isArray(value), 'download list missing')
  assert(value.some((job) => job.comicId === 'macos-replace-1'), 'baseline download job missing')
...
macos-replace-1,macOS Replacement Fixture One,Preview Author,Drama,Preview | Replacement,true,12,120,24,1
macos-replace-2,macOS Replacement Fixture Two,Preview Author,Comedy,Preview | Rollback,false,8,80,18,1
CSV
...
curl --fail --silent   -X POST -H "content-type: application/json" -H "Origin: $URL"   --data '{"scope":"favorites","text":"macOS Replacement Fixture","limit":20}'   "$URL/api/v1/library/query" > "$WORK/query-baseline.json"
...
curl --fail --silent   -X POST -H "content-type: application/json" -H "Origin: $URL"   --data '{"comicIds":["macos-replace-1"]}'   "$URL/api/v1/shelves/$SHELF_ID/items" > "$WORK/shelf-add.json"
curl --fail --silent "$URL/api/v1/shelves/$SHELF_ID" > "$WORK/shelf-baseline.json"
assert_json "$WORK/shelf-baseline.json" shelf

curl --fail --silent   -X POST -H "content-type: application/json" -H "Origin: $URL"   --data '{"comicIds":["macos-replace-1"],"source":"manual","run":false}'   "$URL/api/v1/download" > "$WORK/download-create.json"
curl --fail --silent "$URL/api/v1/downloads" > "$WORK/downloads-baseline.json"
assert_json "$WORK/downloads-baseline.json" downloads

stop_engine
ditto "$DATA_HOME" "$DATA_SNAPSHOT"
...
curl --fail --silent   -X POST -H "content-type: application/json" -H "Origin: $URL"   --data '{"scope":"favorites","text":"macOS Replacement Fixture","limit":20}'   "$URL/api/v1/library/query" > "$WORK/query-candidate.json"
...
rm -rf "$DATA_HOME"
ditto "$DATA_SNAPSHOT" "$DATA_HOME"
install_package "$BASELINE_ROOT"
[[ "$(tr -d '\r\n' < "$INSTALL_ROOT/SOURCE_SHA.txt")" == "$BASELINE_SHA" ]] || fail "baseline application rollback did not take effect"

start_engine
curl --fail --silent   -X POST -H "content-type: application/json" -H "Origin: $URL"   --data '{"scope":"favorites","text":"macOS Replacement Fixture","limit":20}'   "$URL/api/v1/library/query" > "$WORK/query-rollback.json"
...
echo "macOS arm64 preview replacement/rollback acceptance: PASS"
