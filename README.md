...
- **修复 Desktop 推荐偏好消失**：三语改造时 `v5Channels()` 被误写成自递归，导致推荐控制中心渲染到 V5 Shadow 时出现 `Maximum call stack size exceeded`，后续“你的调整”和完整 0–10 控件没有继续绘制。v0.4.6 修复该递归，原有持久化调整无需重建即可重新显示。
- **清理重复重绘**：推荐控制中心只保留一条语言切换重绘路径，避免切换语言时连续重建两次。
- **增加专门防回归**：CI 会扫描本轮本地化动态脚本中的直接自递归 helper，并在真实 Chromium 中注入包含长期画像、V5 Shadow 和持久化人工调整的数据，要求“你的调整”和 0–10 滑杆实际可见。
- **继承 v0.4.5 全部功能**：中 / 日 / 英三语、可跳过新手引导、竖屏扫码、画风任务反馈、Android 书库顶栏优化和保守的 Desktop Node 清理逻辑保持不变。
- **公开升级路径**：Windows v0.4.0 使用 v0.4.6 升级助手一次直达；v0.4.1–v0.4.5 可使用兼容增量更新。Android 正式版为 v49 / 0.4.6。
...
- 面向普通用户的公开升级基线是 v0.4.0。Windows v0.4.0 使用 v0.4.6 Release 中的 **Windows 升级助手** 一次直达最新版，不需要安装任何中间版本。
...
已有 **v0.4.0** 的用户直接下载 `Pica-Library-v0.4.6-upgrade-assistant.zip`，解压后双击 `Upgrade-Pica-Library-v0.4.6.cmd`，即可一次升级到当前最新版；无需安装任何中间版本。助手会保护 `%LOCALAPPDATA%\Pica Library`，并自动完成校验、备份、替换、健康检查与失败回滚。

### Android

Android APK 不通过应用商店分发，请只使用本仓库正式 Release。已有 **v42 / 0.4.0** 的用户可直接通过应用内更新链原地升级到 **v49 / 0.4.4**；无需安装任何中间版本。
