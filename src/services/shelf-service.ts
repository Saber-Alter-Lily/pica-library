...
export class ShelfService {
    ...
    remove(id: string, comicIds: string[]) {
        const shelf = this.database.listShelves().find((item) => item.id === id)
        const result = this.database.removeShelfItems(id, comicIds)
        // Removal changes portable shelf state just like addition does. Touch the
        // shelf revision so a paired phone can distinguish a real Desktop edit
        // from a stale snapshot during two-way reconciliation.
        if (shelf && result.removed > 0) this.database.renameShelf(id, shelf.name)
        return result
    }
}
