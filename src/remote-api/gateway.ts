...
function allowlistedRoute(method: string, pathname: string) {
    ...
    if (
        method === 'GET' &&
        /^\/api\/v1\/shelves\/[^/]+$/.test(pathname)
    )
        return true
    if (
        method === 'GET' &&
        /^\/api\/v1\/comics\/[^/]+$/.test(pathname)
    )
        return true
    if (
        method === 'GET' &&
        /^\/api\/v1\/covers\/[^/]+$/.test(pathname)
    )
        return true
    if (
        method === 'GET' &&
...
export function readRemoteApiToken(file: string) {
        ...
        try {
            ...
            if (window.count > rateLimit) {
                const retryAfter = Math.max(
                    1,
                    Math.ceil(
                        (window.startedAt + rateWindowMs - now) / 1_000
                    )
                )
                response.setHeader('retry-after', String(retryAfter))
                audit(request, pathname, 429)
                return json(response, 429, { error: 'Remote rate limit exceeded' })
            }

            if (!allowlistedRoute(method, pathname)) {
                audit(request, pathname, 404)
                return json(response, 404, { error: 'Remote route unavailable' })
            }
