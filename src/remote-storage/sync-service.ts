    ...
    async sync() {
        ...
        try {
            ...
            for (let comicIndex = 0; comicIndex < local.length; comicIndex++) {
                ...
                // On a brand-new remote library, publish after each fully completed
                // comic. Mobile can start reading completed comics while later ones
                // continue uploading, without ever exposing a half-written comic.
                if (!previous && !same) {
                    emit({
                        phase: 'publishing',
