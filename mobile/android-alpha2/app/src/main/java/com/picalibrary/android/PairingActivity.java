...
public class PairingActivity extends Activity {
    ...
    private void pair(){final String h=host.getText().toString().trim().replaceAll("/$","");final String c=code.getText().toString().trim();if(h.isEmpty()||c.length()!=6){status.setText("请填写电脑地址和 6 位配对码");return;}status.setText("正在连接…");new Thread(()->{try{JSONObject result=BridgeClient.pair(h,c);String token=result.optString("token"),name=result.optString("serverName","Pica Library Desktop");if(token.isEmpty())throw new IllegalStateException("配对失败");BridgeStore.save(this,h,token,name);runOnUiThread(()->{status.setText("配对成功 · 后台正在同步推荐状态");offerFavoriteImport();});try{BridgeClient.syncRecommendationState(this,false);}catch(Exception ignored){}try{ShelfStore.syncWithDesktop(this);}catch(Exception ignored){}SupporterSyncJobs.enqueue(this);}catch(Exception e){runOnUiThread(()->status.setText("连接失败："+(e.getMessage()==null?"请检查地址和配对码":e.getMessage())));}}).start();}
