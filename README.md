...
- **新用户可直接配置推荐**：没有收藏夹也能搜索并添加标签，以 5/10 为中性起点手动调整 0–10；6–10 的正向标签可直接作为首轮推荐召回种子。
- **双端支持开源项目**：Windows / Web 与 Android 设置新增“支持项目”，可前往爱发电自愿支持后续开发、测试和维护；赞助不会解锁额外功能、内容或权限。
- **Windows 更新页修复**：主更新操作、本地 ZIP、进度与状态区域重新分层，解决部分窗口宽度下控件重叠。
- **一次完成升级**：v0.4.1 Windows 可在软件内直接增量升级到 v0.4.2；v0.4.0 可使用 v0.4.2 升级助手直接跨代升级；Android 升级到 versionCode 44 / 0.4.2。
...
**v0.4.1 → v0.4.2** 可直接使用软件内更新。仍停留在 **v0.4.0** 的用户可下载 `Pica-Library-v0.4.2-upgrade-assistant.zip`，解压后双击 `Upgrade-Pica-Library-v0.4.2.cmd` 直接升级到 v0.4.2；助手会保护 `%LOCALAPPDATA%\Pica Library`，并自动完成校验、备份、替换、健康检查与失败回滚。
