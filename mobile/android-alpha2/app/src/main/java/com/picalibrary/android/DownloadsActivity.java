...
public final class DownloadsActivity extends LocaleAwareActivity {
    ...
    private void confirmDelete(PhoneDownloadStore.Comic comic){new AlertDialog.Builder(this).setTitle(LocalizedText.ui("删除手机下载？")).setMessage(LocalizedText.ui("删除手机中的主动下载正文，但不会删除 WebDAV、Desktop、Pica 在线内容或收藏。")) .setNegativeButton(LocalizedText.ui("取消"),null).setPositiveButton(LocalizedText.ui("删除"),(d,w)->{PhoneDownloadStore.remove(this,comic.id);renderList();}).show();}
