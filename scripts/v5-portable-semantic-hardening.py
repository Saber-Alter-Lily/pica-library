...
write('mobile/android-alpha2/app/src/main/java/com/picalibrary/android/RecommendationPolicyStore.java', r'''
...
replace_once(
    ...
    '''        final List<RecommendationItem> items;final String source;final boolean cached;final int batchIndex, maxVisibleBatches;final JSONObject policyBaseline;\n        RecommendationBatch(List<RecommendationItem> items,String source,boolean cached,int batchIndex,int maxVisibleBatches,JSONObject policyBaseline){this.items=items;this.source=source;this.cached=cached;this.batchIndex=batchIndex;this.maxVisibleBatches=maxVisibleBatches;this.policyBaseline=policyBaseline;}'''
)
replace_once(
    'mobile/android-alpha2/app/src/main/java/com/picalibrary/android/BridgeClient.java',
    '''return new RecommendationBatch(out,root.optString("source",""),root.optBoolean("cached",false),root.optInt("batchIndex",0),root.optInt("maxVisibleBatches",Math.max(1,(out.size()+NativeRecommendationPolicy.BATCH_SIZE-1)/NativeRecommendationPolicy.BATCH_SIZE)));''',
    '''return new RecommendationBatch(out,root.optString("source",""),root.optBoolean("cached",false),root.optInt("batchIndex",0),root.optInt("maxVisibleBatches",Math.max(1,(out.size()+NativeRecommendationPolicy.BATCH_SIZE-1)/NativeRecommendationPolicy.BATCH_SIZE)),root.optJSONObject("policyBaseline"));'''
)
replace_once(
    'mobile/android-alpha2/app/src/main/java/com/picalibrary/android/BridgeClient.java',
    '''if(!batch.isEmpty())snapshot.batches.add(batch);NativeRecommendationStore.save(c,snapshot);}return response;}''',
    '''if(!batch.isEmpty())snapshot.batches.add(batch);NativeRecommendationStore.save(c,snapshot);if(desktop.policyBaseline!=null)RecommendationPolicyStore.saveCacheBaseline(c,desktop.policyBaseline);}return response;}'''
)
...
replace_once(
    ...
    '''        NativeRecommendationStore.save(app,snapshot);RecommendationPolicyStore.markCacheBaseline(app);emit(progress,"手机原生 Recommendation V3 已更新",1,1);return snapshot;'''
)

replace_once(
    'mobile/android-alpha2/app/src/main/java/com/picalibrary/android/HomeActivity.java',
    '''pager.addView(compact("上一批",v->{NativeRecommendationStore.previousBatch(this);show();}),new LinearLayout.LayoutParams(0,-2,1));''',
    '''pager.addView(compact("上一批",v->{RecommendationPolicyStore.moveVisibleBatch(this,-1);show();}),new LinearLayout.LayoutParams(0,-2,1));'''
)
replace_once(
    'mobile/android-alpha2/app/src/main/java/com/picalibrary/android/HomeActivity.java',
    '''pager.addView(compact("下一批",v->{NativeRecommendationStore.nextBatch(this);show();}),new LinearLayout.LayoutParams(0,-2,1));''',
    '''pager.addView(compact("下一批",v->{RecommendationPolicyStore.moveVisibleBatch(this,1);show();}),new LinearLayout.LayoutParams(0,-2,1));'''
)

path = Path('test/unit/recommendation-v5-product-contract.test.ts')
text = path.read_text(encoding='utf-8')
old = '''        expect(store).toContain('applyLocalPolicy')\n        expect(cache).toContain('final List<String> tags,categories')'''
new = '''        expect(store).toContain('applyLocalPolicy')\n        expect(store).toContain('adjustment(current,item)-adjustment(baseline,item)')\n        expect(store).toContain('moveVisibleBatch')\n        expect(cache).toContain('final List<String> tags,categories')'''
if old not in text:
    raise SystemExit('V5 product contract local-policy anchor not found')
text = text.replace(old, new, 1)
old = '''        expect(client).toContain('DESKTOP_SYNCED_V5_PORTABLE')'''
new = '''        expect(client).toContain('DESKTOP_SYNCED_V5_PORTABLE')\n        expect(client).toContain('policyBaseline')\n        expect(coordinator).toContain('portableBaseline')'''
if old not in text:
    raise SystemExit('V5 product contract desktop cache anchor not found')
