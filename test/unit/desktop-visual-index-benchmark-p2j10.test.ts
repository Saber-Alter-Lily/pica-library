...
describe('P2 J10 real Visual indexing foreground harness', () => {
    const harness = () =>
        fs.readFileSync(
            'scripts/benchmark/desktop-visual-index-harness.mjs',
            'utf8'
        )
    ...
    it('requires explicit cold model evidence with no synthetic CI inference', () => {
        expect(harness()).toContain('--allow-model-network')
        expect(harness()).toContain('there is no synthetic inference mode')
        expect(harness()).toContain(
            'without observing DINOv2/model artifact network traffic'
        )
        expect(harness()).toContain('waitFixtureIndexed(')
    })

    it('separates cold bootstrap from repeated warm inference foreground metrics', () => {
        expect(harness()).toContain('startWarmSequence(')
        expect(harness()).toContain('warmInferenceRepeats')
    ...
    it('requires foreground samples to remain inside active warm inference', () => {
        expect(harness()).toContain(
            'completed before any full RAF observation window'
        )
        expect(harness()).toContain(
            'completed before any full foreground navigation sample'
        )
        expect(harness()).toContain("afterRaf?.state === 'running'")
    })
})
