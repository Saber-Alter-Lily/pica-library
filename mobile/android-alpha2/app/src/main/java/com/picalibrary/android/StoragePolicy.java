...
final class StoragePolicy {
    ...
    private static synchronized void trim(File root,long limit){
        if(limit==Long.MAX_VALUE||root==null||!root.exists())return;
    ...
    static Usage usage(Context context){
        ...
        long metadata=bytes(root(context,"remote-metadata-v1"))+bytes(MobileStoragePaths.dataFile(context,"favorite-catalog-v1.json"))+bytes(MobileStoragePaths.dataFile(context,"portable-shelves-v1.json"))+bytes(MobileStoragePaths.dataFile(context,"unified-catalog-v1.json"));
        return new Usage(pages,covers,metadata);
    }

    static void maintain(Context context){
        trim(root(context,"reader-pages-v2"),StorageSettings.pageLimitBytes(context));
        long coverLimit=StorageSettings.coverLimitBytes(context);
        trim(root(context,"desktop-cover-cache-v1"),coverLimit);
        trim(root(context,"remote-covers-v1"),coverLimit);
    ...
    static void clearMetadata(Context context){delete(root(context,"remote-metadata-v1"));FavoriteCacheStore.clear(context);File shelf=MobileStoragePaths.dataFile(context,"portable-shelves-v1.json");if(shelf.exists())shelf.delete();}
