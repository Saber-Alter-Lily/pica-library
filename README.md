...
- **下载运行时单实例化**：Desktop 本地下载 runner 只允许一套 scheduler 实例；刷新页面或重复点击“运行队列”不会再叠加第二套下载执行器。
- **下载启动与网页生命周期解耦**：Web 只负责启动后台 runner 并立即返回，不再让一个 HTTP 请求一直挂到整个大队列完成；刷新或关闭下载页不会中断后台下载。
- **异常退出自动恢复下载任务**：程序启动时会识别上次进程中断遗留的 `PREPARING / RUNNING / RETRY_WAIT` 本地任务并恢复为 `QUEUED`，保留已完成页和累计进度；用户主动 `PAUSED` 不会被自动恢复。
- **大队列 UI 保持有界**：取消确认不再读取完整下载队列；列表继续使用 summary + bounded page，1500 个任务场景下只传输当前可见页。
- **降低 SQLite 写入压力**：下载进度写入按节流/合并策略提交，在持续高并发下载时减少频繁同步写库，同时在暂停、失败、完成和退出边界强制 flush 最新进度。
- **恢复状态可见**：页面重开后可看到后台 runner 是否正在运行、是否刚从异常退出恢复，以及当前队列进度；不会再把“后台其实还在跑”表现成无响应。
- **压力回归扩展**：新增 1500 任务 / 350 已完成的大队列长期运行路径、单 runner、异常恢复、写入合并和页面响应性测试。
- **继承 v0.4.7 长任务稳定性能力**：推荐、画风、收藏同步、WebDAV 与 Android 后台任务的暂停 / 继续 / 取消 / 超时 / 恢复机制保持不变。
- **公开升级路径**：Windows v0.4.0 使用 v0.4.8 升级助手一次直达；v0.4.1–v0.4.7 可使用兼容增量更新。Android 正式版为 v51 / 0.4.8。
...
- 面向普通用户的公开升级基线是 v0.4.0。Windows v0.4.0 使用 v0.4.8 Release 中的 **Windows 升级助手** 一次直达最新版，不需要安装任何中间版本。
...
已有 **v0.4.0** 的用户直接下载 `Pica-Library-v0.4.8-upgrade-assistant.zip`，解压后双击 `Upgrade-Pica-Library-v0.4.8.cmd`，即可一次升级到当前最新版；无需安装任何中间版本。助手会保护 `%LOCALAPPDATA%\Pica Library`，并自动完成校验、备份、替换、健康检查与失败回滚。

### Android

Android APK 不通过应用商店分发，请只使用本仓库正式 Release。已有 **v42 / 0.4.0** 的用户可直接通过应用内更新链原地升级到 **v51 / 0.4.8**；无需安装任何中间版本。
