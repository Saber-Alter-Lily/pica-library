...
public final class EhAccountActivity extends LocaleAwareActivity {
    ...
    private void showDisconnectedActions(){String[] labels={"手动导入会话","浏览器登录","注册账号"};new AlertDialog.Builder(this).setTitle(LocalizedText.ui("其他方式")).setItems(labels,(d,w)->{if(w==0)startActivity(new Intent(this,EhSessionActivity.class));else open(w==1?"https://forums.e-hentai.org/index.php?act=Login":"https://forums.e-hentai.org/index.php?act=Reg&CODE=00");}).setNegativeButton(LocalizedText.ui("取消"),null).show();}
    private void showOfficialAccountActions(){String[] labels={"浏览器登录","注册账号"};new AlertDialog.Builder(this).setTitle(LocalizedText.ui("E-H 官方账号")).setItems(labels,(d,w)->open(w==0?"https://forums.e-hentai.org/index.php?act=Login":"https://forums.e-hentai.org/index.php?act=Reg&CODE=00")).setNegativeButton(LocalizedText.ui("取消"),null).show();}
    private void showAccountActions(){String[] labels={"更新网页登录","手动更新会话","重新检查 ExH","清除本机会话"};new AlertDialog.Builder(this).setTitle(LocalizedText.ui("账号管理")).setItems(labels,(d,w)->{if(w==0)startActivity(new Intent(this,EhWebLoginActivity.class));else if(w==1)startActivity(new Intent(this,EhSessionActivity.class));else if(w==2)refreshExh(true);else confirmClearSession();}).setNegativeButton(LocalizedText.ui("取消"),null).show();}
