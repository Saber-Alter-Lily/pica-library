...
public class RemoteStorageActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        super.onCreate(b);getWindow().setStatusBarColor(Ui.BG);getWindow().setNavigationBarColor(Ui.BG);render();
    }
    ...
    private void render(){
        ...
        Button clear=button("清除网盘配置");clear.setOnClickListener(v->{RemoteConfigStore.clear(this);finish();});p.addView(clear);setContentView(scroll);
    }
    ...
    private void test(boolean save){
        ...
        new Thread(()->{try{
            ...
            boolean ok=new RemoteLibraryClient(candidate).test();
            if(save&&ok)RemoteConfigStore.save(this,candidate.baseUrl,candidate.root,candidate.username,candidate.password);
