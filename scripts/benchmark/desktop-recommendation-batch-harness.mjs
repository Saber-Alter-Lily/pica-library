...
function parseOptions() {
    ...
    if (Number(fixture.expectedBatchSize) !== 12)
        throw new Error('J6 fixture must use the production 12-item batch size')
    if (!Array.isArray(fixture.initialBatchComicIds) || fixture.initialBatchComicIds.length !== 12)
        throw new Error('J6 fixture is missing the initial managed batch')
...
function playwright() {
    ...
    if (!toolRoot)
        throw new Error(
            'PICA_PLAYWRIGHT_TOOL_ROOT is required; use the J6 runner so Playwright stays outside project dependencies'
        )
...
async function runRound(chromium, options, index) {
    ...
    try {
        ...
        if (new Set(after.ids).size !== options.fixture.expectedBatchSize)
            throw new Error('J6 next batch contains duplicate card identities')
        if (after.ids.some((id) => before.ids.includes(id)))
            throw new Error('J6 next batch unexpectedly reused current-batch cards')
        if (pageErrors.length)
            throw new Error(
                `Web page error during J6 batch switch: ${pageErrors.join(' | ')}`
            )
...
async function main() {
    ...
    const report = {
        schemaVersion: 1,
        benchmark: 'p2-j6-desktop-recommendation-batch-switch',
