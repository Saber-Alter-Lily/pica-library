...
const records = Array.from({ length: candidateCount }, (_, index) => {
    ...
    return {
        comicId: `j6-recommend-${String(ordinal).padStart(3, '0')}`,
        title: `J6 推荐候选 ${String(ordinal).padStart(3, '0')}`,
        author: `J6 作者 ${ordinal % 9}`,
        description: 'J6 deterministic recommendation batch-switch fixture',
        categories: ['基准'],
...
database.importCatalog(records, 'benchmark:j6')

const profile: FinalLifetimeProfileV3 = {
    schemaVersion: 1,
    profileVersion: FINAL_PROFILE_VERSION,
    registryVersion: 'benchmark:j6',
    favoriteFingerprint: 'benchmark-j6-no-favorites',
...
const versions = {
    ...
    rankerModelVersion: 'benchmark:j6-frozen',
    candidatePoolVersion: 'benchmark:j6-local',
...
const ranked = records.map((record, index) => {
    ...
    if (!comic) throw new Error(`Missing imported J6 candidate ${record.comicId}`)
    return {
        ...
        comic,
        features: {},
        reasons: ['J6_LOCAL_FIXTURE'],
        provenance: [],
        evidence: {
            comicId: record.comicId,
            originIntentIds: [],
            originRouteIds: ['j6-local'],
...
const built: BuiltRecommendationCycleV3 = {
    ...
    ranked,
    readiness: 'READY',
    telemetry: { fixture: 'p2-j6-local-batch-switch' },
    versions
}
...
coordinator.forceNew('p2-j6-fixture')
...
if (!first.cycleId || first.batchIndex !== 0 || first.recommendations?.length !== 12)
    throw new Error('J6 fixture did not create the expected first managed V3 batch')
