...
> 当前研发任务、阶段状态、验收门槛与范围变更统一维护在 [`DEVELOPMENT_TASK_LOG.md`](./DEVELOPMENT_TASK_LOG.md)。未完成任务以该文件为准；本文件不承担前瞻任务跟踪。
...
- “同一作品”识别改为先缩小到同作者作品，再比较标题；标题仍有疑问时再利用封面信息辅助判断，提升跨来源、跨语言和不同上传版本的召回。
- Android 在未连接 Desktop 时也可以基于本地书库独立发现可能的同一作品，不再只依赖电脑同步的已确认关系。
- 大型书库的同作品检查不再受旧的 5000 条读取上限影响，同时保持一次读取完成，避免重复扫描整库。
- 修复 Desktop/Web 从书库、书架、搜索、推荐等位置打开漫画详情后，关闭详情会跳回页面顶部的问题；返回时恢复原浏览位置。
- 继续保持保守合并：疑似关系不会自动写成确认关系，用户明确要求分开的作品仍优先保持分离。
- Desktop v0.4.11 / Android versionCode 54；Windows v0.4.1–v0.4.10 提供兼容增量更新，v0.4.0 使用 v0.4.11 直达升级助手。
...
- Web 下载启动改为 detached background runtime：`/api/v1/downloads/run` 启动后立即返回 runner 状态，不再由长时间 HTTP 请求托管整个队列。
- 程序启动自动恢复异常退出遗留的 LOCAL `PREPARING / RUNNING / RETRY_WAIT` 任务到 `QUEUED`；保留已下载文件、页数、字节与 retry 信息，用户主动 `PAUSED` 不自动恢复。
- 下载页继续使用 summary + bounded page；单任务取消确认改为只读取当前卡片 / 单任务信息，不再走完整 `/api/v1/downloads` 全量队列。
- 下载进度数据库写入增加合并与节流；连续媒体进度不再每个回调都同步写 SQLite，并在暂停、失败、完成、退出等生命周期边界强制 flush。
- 页面重载后显示后台 runner 与恢复状态；runner 的权威状态由后端提供，避免“页面关了以后后台是否还在运行”成为黑盒。
- 大队列回归扩展到 1500 个任务、350 个已完成，并覆盖运行中队列响应性、单实例 runner、异常重启恢复、bounded UI 和进度写入合并。
- v0.4.7 的推荐、画风、收藏同步、WebDAV 与 Android 长任务控制继续保留；本版本不改变这些任务的数据语义。
- Desktop v0.4.8 / Android versionCode 51；Windows v0.4.1–v0.4.7 提供 scoped 增量更新，v0.4.0 使用 v0.4.8 直达升级助手。
...
- 此问题仅影响前端渲染；V5 持久化策略中的 `controls` 仍保存在本地数据库，修复后原有人工调整直接恢复显示，无需重建推荐画像。
- 清理 Recommendation V5 重复的语言切换监听器，避免一次语言变化触发两次面板重建。
- 新增本地化动态 helper 自递归静态审计，并新增真实 Chromium 推荐控制中心成功路径：长期画像、Shadow 通道和持久化人工调整必须同时实际渲染。
- Desktop v0.4.6 / Android versionCode 49；Windows v0.4.1–v0.4.5 提供 scoped 增量更新，v0.4.0 使用 v0.4.6 直达升级助手。

## v0.4.5 — 三语、新手引导与交互收口

- Desktop / Web 与 Android 增加简体中文、日本語、English 三语切换；动态文案、主要设置面板与常用页面纳入真实浏览器 / Android 多宽度布局检查。
- 新增首次进入新版时的可跳过功能引导；支持稍后查看、不再自动提示，并可从设置重新播放。
- Android 配对二维码改为竖屏扫码；说明提示改为更醒目的黄色样式。
- 画风推荐补齐后台运行、进度、完成、部分失败和整体失败反馈；设置保存增加确认缓存与并发回读保护，刷新后保持已确认状态。
- 修复新手引导关闭按钮被主题样式撑大的问题，以及软件更新页标题 / 版本徽标挤叠。
- Android 书库移除右上角三点菜单，把“书架 / 显示 / 刷新”直接放到顶栏，刷新位于最右侧。
- Desktop 浏览器会话增加保守清理：没有已配对手机时，关闭最后页面后延迟清理空闲 Node；存在手机配对时继续保留 Mobile Bridge，不中断手机读取电脑内容。
- Desktop v0.4.5 / Android versionCode 48；Windows v0.4.1–v0.4.4 提供 scoped 增量更新，v0.4.0 继续使用 v0.4.5 直达升级助手。
...
- Desktop v0.4.3 / Android versionCode 45；Windows v0.4.1、v0.4.2 均提供 scoped 增量直升，v0.4.0 提供 v0.4.3 直达升级助手。

## v0.4.2 — 新用户推荐初始化、支持入口与更新体验
...
- 阅读历史从“按章节/Session 多卡片”改为“按漫画聚合一张卡”：显示统一封面、最近阅读章节与页码，并可展开该漫画的章节记录后直接续读指定章节；新记录持久化真实章节标题与章节顺序，旧历史缺少章节顺序时保持向后兼容。
