...
final class ReaderProgress {
    ...
    private String historySessionId="",historyDay="",historyChapterId="",historyStartedAt="",historyChapterTitle="章节";
    ...
    void chapterMetadata(String chapterId,String chapterTitle,int chapterOrder){
        if(chapterId==null||chapterId.isEmpty())return;
        historyChapterId=chapterId;
        historyChapterTitle=chapterTitle==null||chapterTitle.trim().isEmpty()?"章节":chapterTitle.trim();
    ...
    private void recordHistory(String comic,String chapter,int page,String now){LocalDate day=ReadingHistoryStore.localDay(now);String dayKey=day==null?now.substring(0,Math.min(10,now.length())):day.toString();if(historySessionId.isEmpty()||!dayKey.equals(historyDay)||!chapter.equals(historyChapterId)){historySessionId=UUID.randomUUID().toString();historyDay=dayKey;historyChapterId=chapter;historyStartedAt=now;}UnifiedCatalogStore.Entry entry=UnifiedCatalogStore.load(context).byId.get(comic);String title=entry==null?"漫画":entry.title,author=entry==null?"未知作者":entry.displayAuthor(),provider=entry==null?(EhClient.isEhId(comic)?"eh":("pica".equals(source.kind())?"pica":"")):entry.providerId;String chapterTitle=chapter.equals(historyChapterId)?historyChapterTitle:"章节";int chapterOrder=chapter.equals(historyChapterId)?historyChapterOrder:0;ReadingHistoryStore.record(context,historySessionId,comic,chapter,title,author,chapterTitle,chapterOrder,provider,source.kind(),page,historyStartedAt,now);}
