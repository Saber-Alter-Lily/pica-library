# Desktop Recommendation Generation / Batch Benchmark — P2 J6
...
J6 therefore creates two independent harnesses and never reports one as a proxy
for the other.
...
## J6A — deterministic local managed-V3 batch switching
...
Those are owned by J3/J4/J6B respectively.

## J6B — opt-in real Provider regeneration
...
Hosted-runner J6A values are harness-executability evidence, not P2-K budgets.
...
J6B also intentionally does not record Provider URL/account values. A human
...
- the generation precondition exists before Recommendation UI can generate a
  cycle;
- CI executes only the local J6A harness;
...
1. collect representative Windows x64 J3/J4/J5/J6A evidence;
2. collect J6B real-generation evidence with network/provider context;
...
J6 itself selects no performance budget.
