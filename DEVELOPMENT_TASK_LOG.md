...
- **G15 real process-death gate:** a debug-only WorkManager probe is kept RUNNING while the host executes `adb shell am force-stop com.picalibrary.android.dev`. A second instrumentation invocation must use a fresh PID, observe probe runCount >= 2, preserve active/PAUSED recovery state, reconstruct one logical Task Center card per active/paused download, and keep explicitly cancelled history from resurrecting.
...
- G15 closes that named evidence gap with a two-invocation instrumentation scenario and host-side `adb shell am force-stop` while a debug-only WorkManager probe is actually RUNNING.
- The second invocation must run in a fresh PID and observe the probe execute again, proving WorkManager reconstructed unfinished work after the force-stop rather than merely reading the old database row.
