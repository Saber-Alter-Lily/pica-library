...
public final class FavoriteImportWorker extends Worker {
    ...
    @NonNull @Override public Result doWork(){boolean covers=getInputData().getBoolean(KEY_COVERS,false);try{if(covers)setForegroundAsync(foreground("正在导入收藏与封面",0,0));FavoriteCacheStore.Snapshot snapshot=FavoriteCacheStore.syncFromDesktop(getApplicationContext(),covers,(done,total,phase)->{Data data=new Data.Builder().putInt(KEY_DONE,done).putInt(KEY_TOTAL,total).putString(KEY_PHASE,phase).build();setProgressAsync(data);if(covers)setForegroundAsync(foreground(phase,done,total));},()->isStopped());UnifiedCatalogStore.reconcileLocalReferences(getApplicationContext());FavoriteImportJobs.complete(getApplicationContext());return Result.success(new Data.Builder().putInt(KEY_DONE,snapshot.items.size()).putInt(KEY_TOTAL,snapshot.items.size()).putString(KEY_PHASE,covers?"收藏和封面后台导入完成":"收藏后台导入完成").build());}catch(Exception e){if(isStopped())return Result.failure(new Data.Builder().putString(KEY_PHASE,FavoriteImportJobs.paused(getApplicationContext())?"收藏导入已暂停":"收藏导入已取消").build());if(getRunAttemptCount()<3)return Result.retry();return Result.failure(new Data.Builder().putString(KEY_PHASE,e.getMessage()==null?"收藏导入失败":e.getMessage()).build());}}
