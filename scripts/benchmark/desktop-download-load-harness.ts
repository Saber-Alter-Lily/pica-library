...
function options(args = process.argv.slice(2)): J8Options {
...
function desktopController() {
    return {
        csrfToken: 'j8-local-fixture-csrf',
        configured: () => true,
        status: () => ({ benchmarkFixture: 'p2-j8-local-download-load' }),
        save: async () => ({}),
...
async function waitForDownloadTerminal(
    ...
    throw new Error('Timed out waiting for J8 fixture download to finish')
}

async function main() {
    const config = options()
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'pica-j8-download-load-'))
    ...
    const provider = {
        comicInfo: async () => ({
            _id: 'j8-load-comic',
            title: 'J8 Controlled Download Load',
            author: 'P2 J8 Fixture',
        ...
        picturesAll: async () =>
            Array.from({ length: config.pages }, (_, index) => ({
                ...
                url: `https://fixture.invalid/j8/${index + 1}.jpg`,
                media: {
                    path: `j8/${index + 1}.jpg`,
    ...
    let jobId = ''
    try {
        const job = service.enqueueDownload({ comicId: 'j8-load-comic' })
        jobId = job.id
        ...
        if (!queue.started) throw new Error('J8 local download queue did not start')
        ...
        if (terminal.status !== 'COMPLETED')
            throw new Error(
                `J8 fixture download ended in ${terminal.status}: ${terminal.error ?? 'unknown error'}`
            )
