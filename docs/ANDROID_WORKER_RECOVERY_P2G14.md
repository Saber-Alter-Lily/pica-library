# Android Worker Process-Death Recovery — P2 G14
...
G13 gave Task Center a durable current-work identity. G14 adds the missing process-death evidence and fixes one terminal-state leak found while building that evidence.
...
G14 intentionally reuses established Android infrastructure rather than introducing another scheduler or process supervisor.
...
The production scheduler remains WorkManager. G14 adds only recovery identity, terminal cleanup and test evidence around that scheduler.
...
G14 adds terminal identity cleanup for the three singleton families.
...
## Recovery matrix covered by G14
...
G14 is real process-death evidence, but it is not the final Android runtime gate.
