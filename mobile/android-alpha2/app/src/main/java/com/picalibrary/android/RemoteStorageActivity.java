...
public class RemoteStorageActivity extends Activity {
    ...
    private void test(boolean save){
        ...
        new Thread(()->{try{
            ...
            final String suffix=portable;runOnUiThread(()->{status.setText(ok?(save?"WebDAV 已保存并连接成功"+suffix:"WebDAV 连接成功"):("WebDAV 服务未返回可用状态"));if(save&&ok){password.setText("");user.setText("");}});
    ...
    private void refreshPortableState(){
        ...
        status.setText("正在刷新便携状态…");new Thread(()->{try{FavoriteCacheStore.Snapshot snapshot=FavoriteCacheStore.syncFromRemote(this);runOnUiThread(()->status.setText("便携状态已刷新 · 收藏 "+snapshot.items.size()+" 本 · 阅读进度会在阅读时自动同步"));}catch(Exception e){runOnUiThread(()->status.setText("便携状态刷新失败："+(e.getMessage()==null?"尚未由电脑发布":e.getMessage())));}}).start();
    }
