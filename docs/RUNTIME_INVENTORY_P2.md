...
| RT-11 | Recommendation V5 shadow retrieval | **H2A in progress:** detach manual Desktop POST into service-owned background task while preserving the existing computation core | candidate pool/audit telemetry persists only after final checkpoint; serving remains unchanged | target: explicit confirmation + status + pause/resume/cancel + provider-route checkpoints | NET-P, CPU, DB-R/W | **HIGH / ACTIVE REMEDIATION** |
